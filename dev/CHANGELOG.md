...
- Added test email and test notification functionality to Backup Notifications settings page:
  ...
  - Added "qr-code" icon button to "Additional NTFY Topic" field that displays a QR code for the topic using the existing NtfyQrModal component
  - Icons are available in both the expanded row view (desktop) and accordion view (mobile)
  - Same functionality is also available in the bulk edit modal for testing before applying to multiple backups
  - Test email endpoint now accepts an optional `toEmail` parameter to send test emails to specific addresses
  - Icons follow the same design pattern as the "Filter by Server Name" input box with proper spacing and hover states
- Added "Last Backup Timestamp (DB)" column to the CSV file generated by the "Download CSV" button in the overdue monitoring settings page. This column shows the actual last backup timestamp from the database, allowing users to compare it with the configured "Last Backup" timestamp to verify they match.
...
### Fixed
- Fixed audit log timestamps displaying in GMT instead of browser timezone: Timestamps in the audit log viewer now correctly display in the user's browser timezone. SQLite timestamps (stored in UTC as "YYYY-MM-DD HH:MM:SS") are now properly parsed as UTC and converted to the browser's local timezone for display. Added `formatSQLiteTimestamp()` utility function and updated `formatRelativeTime()` to handle SQLite timestamp format correctly.
- Fixed email and NTFY failure audit logging: Email and NTFY notification sending failures (including SMTP connection failures, email delivery failures, NTFY connection failures, and NTFY delivery failures) are now properly logged to the audit log. The system now always attempts to log failures even if SMTP configuration is unavailable, ensuring all connection and delivery failures are recorded in the audit log for troubleshooting. All audit logging operations are wrapped in try-catch blocks to prevent audit logging errors from breaking notification flows, and the system attempts to retrieve SMTP configuration if it's not immediately available when logging failures.
