...
export function OpenServerConfigButton() {
  ...
  const handleServerClick = (serverUrl: string) => {
    try {
      ...
      toast({
        title: t("Error"),
        description: t("Failed to open server URL"),
        variant: "destructive",
        duration: 3000,
      });
    }
  ...
  const handleServerContextMenu = (e: React.MouseEvent, serverUrl: string) => {
    ...
    try {
      ...
      toast({
        title: t("Error"),
        description: t("Failed to open old UI"),
        variant: "destructive",
        duration: 3000,
      });
    }
    ...
    >
      <PopoverTrigger asChild>
        <Button
          ...
          disabled={isLoading}
          title={t("Duplicati configuration")}
        ...
        <div className="grid gap-4">
          <GradientCardHeader>
            <h4 className="text-lg font-semibold leading-none text-white">{t("Open Duplicati Configuration")}</h4>
          </GradientCardHeader>
          <div className="px-1 -mt-2">
            <p className="text-xs text-muted-foreground">
               {t("Select a server below to manage its settings and backups")}
            </p>
          </div>
              ...
              {isLoading ? (
                ...
                serverAddresses.map((server: ServerAddress) => (
                  <button
                    ...
                    title={t("Open {{serverName}} configuration (Right-click for old UI)", {
                      serverName: server.alias ? `${server.alias} (${server.name})` : server.name,
                    })}
                  >
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <span className="text-sm font-medium">{t("Configure server addresses")}</span>
            </button>
          </div>
        </div>
      </PopoverContent>
