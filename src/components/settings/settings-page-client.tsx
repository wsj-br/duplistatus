...
export function SettingsPageClient({ currentUser }: SettingsPageClientProps) {
  ...
  useEffect(() => {
    ...
    if (currentServerListHash !== lastServerListHash) {
      refreshServerListOnly().then(() => {
        setLastServerListHash(currentServerListHash);
        toast({
          title: t("Server List Updated"),
          description: t("New servers detected and added to the list"),
          duration: 3000
        });
      }).catch((error) => {
    ...
    return (
      <div className="flex items-center justify-center py-8">
        <div className="text-center">
          <div className="text-lg font-medium">{t("Loading configuration...")}</div>
        </div>
      </div>
    );
  }
  ...
  if (!config) {
    return (
      <div className="flex items-center justify-center py-8">
        <div className="text-center">
          <div className="text-lg font-medium text-red-600">{t("No configuration available")}</div>
        </div>
      </div>
    );
  }
          ...
          <div className={`${isSidebarCollapsed ? 'px-2' : 'px-4'} py-4 border-b border-border sticky top-0 bg-background z-10 relative h-[73px] flex items-center`}>
            <div className={`flex items-center w-full gap-3 ${isSidebarCollapsed ? 'justify-center' : 'justify-between'}`}>
              <div className={`flex items-center gap-3 min-w-0 ${isSidebarCollapsed ? 'justify-center' : ''}`}>
                ...
                <h2 className={`text-lg font-semibold truncate whitespace-nowrap overflow-hidden transition-all duration-300 ${isSidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-[240px] opacity-100'}`}>{t("Settings")}</h2>
              </div>
              {!isSidebarCollapsed && (
                <button
              ...
              <div>
                ...
                <div className="space-y-1">
                  <button
                    ...
                    title={isSidebarCollapsed ? t("Backup Notifications") : undefined}
                  >
                    <Bell className="h-4 w-4 flex-shrink-0" />
                    <span className={`whitespace-nowrap overflow-hidden transition-all duration-300 ${activeSection === 'notifications' ? 'font-medium' : ''} ${isSidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-[240px] opacity-100'}`}>{t("Backup Notifications")}</span>
                  </button>
                  <button
                    ...
                    title={isSidebarCollapsed ? t("Backup Monitoring") : undefined}
                  >
                  ...
                  <button
                    ...
                    title={isSidebarCollapsed ? t("Templates") : undefined}
                  >
                    <FileText className="h-4 w-4 flex-shrink-0" />
                    <span className={`whitespace-nowrap overflow-hidden transition-all duration-300 ${activeSection === 'templates' ? 'font-medium' : ''} ${isSidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-[240px] opacity-100'}`}>{t("Templates")}</span>
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  <button
                    ...
                    title={isSidebarCollapsed ? t("NTFY") : undefined}
                  >
                    <MessageSquare className={`h-4 w-4 flex-shrink-0 ${isNtfyConfigValid() ? 'text-green-600' : 'text-yellow-500'}`} />
                    <span className={`whitespace-nowrap overflow-hidden transition-all duration-300 ${activeSection === 'ntfy' ? 'font-medium' : ''} ${isSidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-[240px] opacity-100'}`}>{t("NTFY")}</span>
                  </button>
                  <button
                    ...
                    title={isSidebarCollapsed ? t("Email") : undefined}
                  >
                    <Mail className={`h-4 w-4 flex-shrink-0 ${isEmailConfigValid() ? 'text-green-600' : 'text-yellow-500'}`} />
                    <span className={`whitespace-nowrap overflow-hidden transition-all duration-300 ${activeSection === 'email' ? 'font-medium' : ''} ${isSidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-[240px] opacity-100'}`}>{t("Email")}</span>
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1">
                  <button
                    ...
                    title={isSidebarCollapsed ? t("Servers") : undefined}
                  >
                    <Server className="h-4 w-4 flex-shrink-0" />
                    <span className={`whitespace-nowrap overflow-hidden transition-all duration-300 ${activeSection === 'server' ? 'font-medium' : ''} ${isSidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-[240px] opacity-100'}`}>{t("Servers")}</span>
                  </button>
                  <button
                    ...
                    title={isSidebarCollapsed ? t("Display Settings") : undefined}
                  >
                  ...
                  {currentUser?.isAdmin && (
                    <button
                      ...
                      title={isSidebarCollapsed ? t("Database Maintenance") : undefined}
                    >
                      <Database className="h-4 w-4 flex-shrink-0" />
                      <span className={`whitespace-nowrap overflow-hidden transition-all duration-300 ${activeSection === 'database-maintenance' ? 'font-medium' : ''} ${isSidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-[240px] opacity-100'}`}>{t("Database Maintenance")}</span>
                    </button>
                  )}
                  {currentUser?.isAdmin && (
                    <button
                      ...
                      title={isSidebarCollapsed ? t("Users") : undefined}
                    >
                      <Users className="h-4 w-4 flex-shrink-0" />
                      <span className={`whitespace-nowrap overflow-hidden transition-all duration-300 ${activeSection === 'users' ? 'font-medium' : ''} ${isSidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-[240px] opacity-100'}`}>{t("Users")}</span>
                    </button>
                  )}
                  <button
                    ...
                    title={isSidebarCollapsed ? t("Audit Log") : undefined}
                  >
                    <ScrollText className="h-4 w-4 flex-shrink-0" />
                    <span className={`whitespace-nowrap overflow-hidden transition-all duration-300 ${activeSection === 'audit' ? 'font-medium' : ''} ${isSidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-[240px] opacity-100'}`}>{t("Audit Log")}</span>
                  </button>
                  {currentUser?.isAdmin && (
                    <button
                      ...
                      title={isSidebarCollapsed ? t("Audit Log Retention") : undefined}
                    >
                      <Clock className="h-4 w-4 flex-shrink-0" />
                      <span className={`whitespace-nowrap overflow-hidden transition-all duration-300 ${activeSection === 'audit-retention' ? 'font-medium' : ''} ${isSidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-[240px] opacity-100'}`}>{t("Audit Log Retention")}</span>
                    </button>
                  )}
                  {currentUser?.isAdmin && (
                    <button
                      ...
                      title={isSidebarCollapsed ? t("Application Logs") : undefined}
                    >
                      <Terminal className="h-4 w-4 flex-shrink-0" />
                      <span className={`whitespace-nowrap overflow-hidden transition-all duration-300 ${activeSection === 'application-logs' ? 'font-medium' : ''} ${isSidebarCollapsed ? 'max-w-0 opacity-0' : 'max-w-[240px] opacity-100'}`}>{t("Application Logs")}</span>
                    </button>
                  )}
                </div>
              </div>
                ...
                <div className="mt-16 px-3">
                  <div className="rounded-md bg-muted/50 px-3 py-2 border border-border/50">
                    <p className="text-xs text-muted-foreground">
                      {t("Note: Settings are read-only. Some features like test notifications remain available.")}
                    </p>
                  </div>
                </div>
              )}
