...
export async function generateMetadata() {
  // Note: Metadata is handled at the layout level for i18n
  // This page doesn't render content directly, so metadata is set in layout
  return {
    other: {
...
export default async function DashboardPage() {
  ...
  const serversSummary = await getServersSummary(access);
  const overallSummary = await getOverallSummaryFromServers(serversSummary);
  const allServersChartData = await getAggregatedChartData(access);
