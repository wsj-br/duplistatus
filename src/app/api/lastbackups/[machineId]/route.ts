...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Get all unique backup names for this machine
    const backupNames = dbOps.getMachineBackups.all(machine.id) as BackupRecord[];
    ...
    for (const backupName of uniqueBackupNames) {
      const latestBackup = dbOps.getLatestBackupByName.get(machine.id, backupName) as BackupRecord | undefined;
      if (latestBackup) {
    ...
    console.error('Error fetching machine backup details:', error instanceof Error ? error.message : String(error));
