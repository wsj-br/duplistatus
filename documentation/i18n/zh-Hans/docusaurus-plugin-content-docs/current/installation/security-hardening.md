...
**duplistatus** 的生产环境加固是分层且可选的。此处描述的每个功能默认都是关闭的，因此新安装后可以继续工作，直到您选择启用它。有三个独立的层级：
...
2. 创建 [API 密钥](#api-keys) 并启用 **要求外部 API 使用 API 密钥**。这无需反向代理即可工作，是最快速的改进。
3. 通过 [具有 HTTPS 的反向代理](#https-with-a-reverse-proxy) 提供 **duplistatus** 服务。
4. 如果打算使用白名单，请将代理的 TCP 对等地址添加到 **受信任的代理**（或 `IP_TRUSTED_PROXIES`）。
5. 可选择启用管理界面和外部 [IP 白名单](#ip-allowlist)，使用 **检测到的 IP** 和最近 IP 建议以避免将自己锁定。
...
Caddy 的 `reverse_proxy` 指令为您设置客户端 IP 头。使用 IP 允许列表时，您仍必须在 **受信任的代理** 下列出代理的 TCP 对等地址（参见[下文](#trusted-proxies-for-ip-allowlists)）。
...
启用[IP 允许列表](../user-guide/settings/ip-allowlist-settings.md)后，在 **受信任的代理** 下列出代理（或设置 `IP_TRUSTED_PROXIES`）。应用程序仅在 TCP 对等方是受信任代理时才遵循 `X-Forwarded-For` 和 `X-Real-IP`；否则忽略它们。
