...
- **通知与警报**：集成NTFY和SMTP电子邮件支持，用于备份警报，包括过期备份通知。
- **用户管理**：使用管理员和用户角色登录，可配置的密码策略，账户锁定和用户管理。
- **安全加固**：可选的额外保护，用于Duplicati上传和主页小部件的API密钥（带有上传大小和速率限制），管理界面和外部API的独立IP允许列表，防欺骗保护，以及HTTPS反向代理指南。
- **审计日志**：记录所有系统更改和用户操作的完整审计轨迹，具有高级过滤、导出功能和可配置的保留期限。
- **应用程序日志查看器**：仅管理员可通过 Web 界面查看、搜索和导出应用程序日志，具有实时监控功能。
- **多语言支持**：界面和文档支持英语、法语、德语、西班牙语、巴西葡萄牙语、印地语和简体中文。
...
- API密钥和IP允许列表的想法/实现由`henmohr`在问题[#79](https://github.com/wsj-br/duplistatus/issues/79)中提出
- Duplicati SVG图标来自https://dashboardicons.com/icons/duplicati
- ntfy SVG图标来自https://dashboardicons.com/icons/ntfy
- GitHub SVG图标来自https://github.com/logos
