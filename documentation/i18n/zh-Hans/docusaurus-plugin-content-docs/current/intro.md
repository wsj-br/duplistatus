...
- **通知和警报**：集成 NTFY 和 SMTP 电子邮件支持用于备份警报，包括逾期备份通知。
- **用户管理**：具有管理员和用户角色的登录，可配置密码策略、账户锁定和用户管理。
- **安全加固**：可选的额外保护，为 Duplicati 上传和首页小部件提供 API 密钥（带上传大小和速率限制），为管理界面和外部 API 提供独立的 IP 白名单，防欺骗保护，以及 HTTPS 反向代理指南。
- **审计日志**：完整记录所有系统更改和用户操作的审计轨迹，具有高级过滤、导出功能和可配置的保留期限。
