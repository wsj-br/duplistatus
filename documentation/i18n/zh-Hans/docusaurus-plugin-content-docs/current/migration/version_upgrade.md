...
### 如果你运行的是 1.2.1 或更高版本 {/* #if-youre-running-version-121-or-later */}
...
### 如果你运行的是 1.2.1 之前的版本 {/* #if-youre-running-a-version-before-121 */}
...
如果你使用的是 Linux，不必担心要启动辅助容器。你可以使用原生的 `cp` 命令将数据库直接从正在运行的容器提取到你的主机上。
...
如果你在 Windows 上运行 Docker Desktop，有两种简单的方法可以在不使用命令行的情况下处理此操作：

###### 选项 A：使用 Docker Desktop（最简单） {/* #option-a-use-docker-desktop-easiest */}
1. 打开 Docker Desktop 仪表板。
2. 转到“容器”选项卡并点击你的 duplistatus 容器。
3. 点击“文件”选项卡。
4. 导航到 `/app/data/`。
5. 右键点击 `backups.db` 并选择 **另存为...** 将其下载到您的 Windows 文件夹中。

###### 选项 B：使用 PowerShell {/* #option-b-use-powershell */}
如果你更喜欢使用终端，可以使用 PowerShell 将文件复制到你的桌面：

```powershell
docker cp duplistatus:/app/data/backups.db $HOME\Desktop\duplistatus-backup.db
```

##### 如果你使用绑定挂载 {/* #if-you-use-bind-mounts */}
如果你最初使用绑定挂载设置了容器（例如，你将像 `/opt/duplistatus` 这样的本地文件夹映射到容器），你根本不需要 Docker 命令。只需使用你的文件管理器复制文件：
- Linux：`cp /path/to/your/folder/backups.db ~/backups.db`
- Windows：只需在 **文件资源管理器** 中从你在设置期间指定的文件夹中复制文件。

#### 恢复你的数据 {/* #restoring-your-data */}
如果你需要从之前的备份恢复数据库，请根据你的操作系统按照以下步骤操作。
...
如果你使用的是 Docker Desktop，可以通过 GUI 或 PowerShell 执行恢复。

###### 选项 A：使用 Docker Desktop（GUI） {/* #option-a-use-docker-desktop-gui */}
1. 确保 duplistatus 容器正在运行（Docker Desktop 要求容器处于活动状态才能通过 GUI 上传文件）。
2. 转到容器设置中的“文件”选项卡。
3. 导航到 `/app/data/`。
...
##### 如果你使用绑定挂载 {/* #if-you-use-bind-mounts-1 */}
如果你使用的是映射到容器的本地文件夹，则不需要任何特殊命令。
...
### 升级到 0.9.x 或更高版本（Schema v4.0） {/* #upgrading-to-version-09x-or-later-schema-v40 */}

:::warning
**现在需要身份验证。** 升级后，所有用户必须登录。
:::

#### 自动变更的内容 {/* #what-changes-automatically */}
...
#### 自动变更的内容 {/* #what-changes-automatically-1 */}
...
#### 自动变更的内容 {/* #what-changes-automatically-2 */}
