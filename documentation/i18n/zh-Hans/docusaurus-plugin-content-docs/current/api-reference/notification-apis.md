...
- **注意事项**:
  ...
  - 模板测试将通知发送到 NTFY 和电子邮件（如果已配置）
  - 电子邮件测试需要设置 SMTP 配置
  - 测试电子邮件端点在读取 SMTP 配置之前清除请求缓存，确保外部脚本可以更新配置并立即反映在测试电子邮件中
  - 模板测试和每日摘要发送立即绕过每个备份的抑制

## 预览通知模板 - `/api/notifications/preview` {/* #preview-notification-template---apinotificationspreview */}
- **端点**: `/api/notifications/preview`
- **方法**: POST
- **描述**: 使用生产 Markdown 渲染器渲染通知模板而不发送。正文包括 `kind` (`success`、`warning`、`overdueBackup` 或 `dailySummaryEmail`) 和正在编辑的模板。每日摘要预览使用当前实际快照；其他类型使用确定性示例值。电子邮件 HTML 旨在用于沙盒 iframe。成功、警告/错误和过期备份也返回 NTFY 有效载荷 (`ntfyMessage`); 任何 GFM 表头将被省略，正文行将是纯文本。
- **认证**: 需要有效的会话和 CSRF 令牌
