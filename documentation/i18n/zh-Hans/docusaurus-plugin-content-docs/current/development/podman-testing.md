...
- `check.duplistatus`：检查日志、连接和应用程序健康状况。

## 调试命令 {/* #debugging-commands */}

- `logs.duplistatus`：显示 pod 的日志。
- `exec.shell.duplistatus`：在容器中打开 shell。
- `restart.duplistatus`：停止 pod，删除容器，复制镜像，创建容器，并启动 pod。

## 使用工作流程 {/* #usage-workflow */}
...
1. 传输 Docker 镜像：
   ...
   - 如果从远程开发服务器复制，使用 `./copy.docker.duplistatus.remote`（需要包含 `.env` 和 `REMOTE_USER` 的 `REMOTE_HOST` 文件）
2. 使用 `./start.duplistatus` 启动容器（独立，无根）
   - 或者使用 `./pod.testing` 以 pod 模式测试（具有根权限）
3. 使用 `./check.duplistatus` 和 `./logs.duplistatus` 进行监控
4. 完成后使用 `./stop.duplistatus` 停止
5. 使用 `./restart.duplistatus` 进行完整重启周期（停止，复制镜像，启动）
...
- **独立模式**（`start.duplistatus`）：以 rootless 方式运行，使用 `--userns=keep-id`
- **Pod 模式**（`pod.testing`）：在 Pod 内以 root 方式运行以进行测试

两种模式均可与自动 DNS 检测正常工作。

## 环境配置 {/* #environment-configuration */}

`copy.docker.duplistatus.local` 和 `copy.docker.duplistatus.remote` 都需要 `.env` 文件位于 `scripts/podman_testing` 目录中：
...
`start.duplistatus` 脚本需要一个 `.env` 文件，其中至少包含 `IMAGE` 变量：

```
IMAGE=wsj-br/duplistatus:devel
```

**注意**：脚本的错误消息提到 `REMOTE_USER` 和 `REMOTE_HOST`，但这些实际上并未被 `start.duplistatus` 使用—仅需要 `IMAGE`。
