...
- **备用支持**：在非 systemd 系统上回退到解析 `/etc/resolv.conf`
...
2. 使用 `./start.duplistatus` 启动容器（独立，无根）
   - 或使用 `./pod.testing` 在 pod 模式下测试（带 root 权限）
...
如果在相同机器上运行 Podman 服务器，请使用 `http://localhost:9666`。

如果在其他服务器上，请使用以下命令获取 URL：
...
- **独立模式**（`start.duplistatus`）：以无根模式运行，使用 `--userns=keep-id`
- **Pod 模式**（`pod.testing`）：在 pod 内部以根用户身份运行，用于测试目的

两种模式都可与自动 DNS 检测正常工作。

## 环境配置 {/* #environment-configuration */}

`copy.docker.duplistatus.local` 和 `copy.docker.duplistatus.remote` 都需要在 `scripts/podman_testing` 目录中有一个 `.env` 文件：
...
**注意**：脚本的错误消息提到了 `REMOTE_USER` 和 `REMOTE_HOST`，但这些实际上并未被 `start.duplistatus` 使用——仅需要 `IMAGE`。
