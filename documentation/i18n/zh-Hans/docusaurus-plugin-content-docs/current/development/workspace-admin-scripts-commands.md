...
删除所有构建产物、node_modules 目录和其他生成的文件以确保干净状态。当您需要执行全新安装或解决依赖问题时，这很有用。该命令将删除：
- `node_modules/` 目录
- `.next/` 构建目录
- `dist/` 目录
- `out/` 目录
...
或者使用自动化脚本（首选 `source` 以便 **nvm** 应用于当前 shell；对于 **CI** 或非交互式运行，请使用 `CI=1` 或 `UPGRADE_ALLOW_EXEC=1`）：

```bash
source ./scripts/upgrade-dependencies.sh
```

`upgrade-dependencies.sh` 脚本使整个依赖升级过程自动化。它是项目无关的：包管理器、工作区包和每个包的验证命令都是自动检测的（因此根目录和 `documentation/` 包都会升级，没有硬编码路径）。它：
- 通过 `upgrade-tools.sh` 源工具设置（nvm / Node LTS、全局 `pnpm`、`npm-check-updates`、`doctoc`）
- 对每个包执行 **build-safe** 升级：`npm-check-updates` 解析最新版本，然后从工作区根目录进行安装和 `typecheck`/`lint` 运行。验证失败的升级通过编辑 `package.json` 进行二分（而不是 `pnpm add`，pnpm 在工作区根目录拒绝该文件）。嵌入的同级网关在 `eslint-plugin-react` / `typescript-eslint` 尚不允许最新主要版本时固定 `eslint` 和 `typescript`。
- 更新工作区 pnpm 锁定文件并安装依赖项
- 更新 browserslist 数据库
- 检查漏洞（`pnpm audit`）并应用非破坏性修复（`pnpm audit --fix`）
- **优先考虑安全**：如果易受攻击的直接依赖项只能通过破坏构建的升级来修复，则强制应用安全版本并报告构建错误，以便代码可以更新以实现兼容性
- 打印摘要（已升级与跳过的破坏构建的包、已修复/剩余的漏洞，以及用于手动回滚的清单快照路径）
- 使用 `/usr/bin/cp` 复制 `package.json` 和锁定文件，这样源交互式 `cp` 别名（例如 `cp -i`）不会提示覆盖这些文件
...
## 切换本地或 npm ai-i18n-tools {/* #switch-local-or-npm-ai-i18n-tools */}
...
将此仓库指向同级的 [ai-i18n-tools](https://github.com/wsj-br/ai-i18n-tools) 检出或返回到发布的 npm 包，然后打印解析后的版本。`--local` 写入 `link:../ai-i18n-tools` （使用 `--path` 或 `AI_I18N_TOOLS_PATH` 覆盖路径），因此 `pnpm i18n:*` 和 `ai-i18n-tools/runtime` 都使用该树。`--remote` 安装最新 npm 版本作为 `^x.y.z`。请勿提交 `link:` 说明符。
...
此脚本比较您的开发环境和正在运行的 Docker 容器之间的版本。它：
- 仅按主版本比较 SQLite 版本（例如，3.45.1 与 3.51.1 被视为兼容，显示为"✅ (major)"）
- 精确比较 Node、npm 和 duplistatus 版本（必须完全匹配）
- 显示显示所有版本比较的格式化表格
- 提供带有颜色编码结果的摘要（✅ 表示匹配，❌ 表示不匹配）
- 如果所有版本都匹配则以代码 0 退出，如果有不匹配则以代码 1 退出
...
以格式化表格显示配置表中 `backup_settings` 值的内容。用于调试通知配置很有用。默认数据库路径：`data/backups.db`。
