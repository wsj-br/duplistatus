...
- Next.js 独立输出（`output: 'standalone'`）与容器入口点启动 `server.js`
- Docker（基于 node:alpine）支持多架构构建（AMD64、ARM64）。镜像仅构建 Next.js 应用（不构建 Docusaurus 站点）；pnpm 版本从 `packageManager` 中的 `package.json` 获取
