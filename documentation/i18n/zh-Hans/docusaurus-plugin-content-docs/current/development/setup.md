...
### 3. 移除旧的 Node.js 安装（如果您已经安装了） {/* #3-remove-old-nodejs-installations-if-you-already-have-it-installed */}
...
### 6. 在仓库根目录下创建包含这些变量的 `.env` 文件。 {/* #6-create-the-env-file-at-the-repository-basedir-with-these-variables */}
...
- `pnpm dev` - 通过 `concurrently` 同时启动 Next.js 开发服务器（端口 8666）和 cron 服务（端口 8667）（包含预检查）。按 CTRL-C 停止两者。Next.js 的 `NODE_OPTIONS` 加载 `scripts/dev-preload.cjs`，该脚本应用 `scripts/peer-ip.cjs`（用于 IP 允许列表的 TCP 对端地址）和请求日志时间戳。
- `pnpm dev:next` - 仅在端口 8666 上启动 Next.js 开发服务器（无 cron）。
- `pnpm build` - 构建用于生产环境的应用程序（包含预检查）
...
- `pnpm build-local` - 构建并为本地生产环境做准备（包含预检查，将静态文件复制到 standalone 目录）
- `pnpm start-local` - 在本地启动生产服务器（端口 8666，包含预检查）。**注意：** 先运行 `pnpm build-local`。使用 `--require ./scripts/peer-ip.cjs` 启动 standalone 服务器。
- `pnpm start` - 使用相同的 peer-ip preload 启动生产服务器（端口 9666）。Docker 使用 `docker-entrypoint.sh` 加载相同的脚本。

### Docker 脚本 {/* #docker-scripts */}
- `pnpm docker:up` - 启动 Docker Compose 栈
- `pnpm docker:down` - 停止 Docker Compose 栈
...
- `pnpm cron:dev` - 仅在开发模式下启动带有文件监视功能的 cron 服务（端口 8667）。使用 `pnpm dev` 时通常不需要，因为它已经启动了 cron。
