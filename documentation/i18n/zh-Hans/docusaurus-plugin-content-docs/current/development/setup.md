...
- Node.js（参见 `engines.node` 中的 `package.json`）
- pnpm（参见 `engines.pnpm` / `packageManager` 中的 `package.json`）
- SQLite3
- Inkscape（用于文档 SVG 翻译和 PNG 导出；仅在运行 `translate` 或 `translate:svg` 时需要）
- bat/batcat（用于显示漂亮的 `translate:help` 版本）
...
### 3. 删除旧的 Node.js 安装（如果已安装） {/* #3-remove-old-nodejs-installations-if-you-already-have-it-installed */}
...
项目包含几个用于不同开发任务的 npm 脚本：

### 开发脚本 {/* #development-scripts */}
- `pnpm dev` - 同时启动 Next.js 开发服务器（端口 8666）和 cron 服务（端口 8667），通过 `concurrently` 运行（包含预检查）。按 CTRL-C 可停止两者。Next.js 的 `NODE_OPTIONS` 加载 `scripts/dev-preload.cjs`，应用 `scripts/peer-ip.cjs`（IP 白名单的 TCP 对等地址）和请求日志时间戳。
- `pnpm dev:next` - 仅在端口 8666 上启动 Next.js 开发服务器（无 cron）。
- `pnpm build` - 构建用于生产的应用程序（包含预检查）
- `pnpm lint` - 运行 ESLint 检查代码质量
- `pnpm typecheck` - 运行 TypeScript 类型检查
- `scripts/upgrade-dependencies.sh` — 安全构建升级所有工作区包（自动检测）。使用 `npm-check-updates` 解析最新版本，在工作区根目录安装，并且只保留能通过每个包的 `typecheck`/`lint` 的升级（对等门钉住 `eslint` / `typescript` 当 lint 栈不允许最新的主要版本时）。然后运行 `pnpm audit` / `audit --fix` 并强制应用（并报告）任何需要代码更改的安全修复。刷新工作区锁定文件和 browserslist。建议使用 `source ./scripts/upgrade-dependencies.sh` 以便 **nvm** 应用于您的 shell；在 CI 或自动化中直接执行文件时使用 `CI=1` 或 `UPGRADE_ALLOW_EXEC=1`。另请参见仅限 Node/pnpm 工具的 `scripts/upgrade-tools.sh`。
- `scripts/clean-workspace.sh` - 清理工作区
- `pnpm i18n:tools --local` / `--remote` — 链接同级 `ai-i18n-tools` 检出或恢复最新的 npm 包（`scripts/link-ai-i18n-tools.sh`）。请勿提交 `link:` 说明符。
...
- `pnpm start` - 在生产模式下构建并提供文档站点（默认端口 3000）
- `pnpm start:en` - 以英语启动文档开发服务器（启用热重载）
- `pnpm start:fr` - 以法语区域设置启动文档开发服务器（启用热重载）
- `pnpm start:de` - 以德语区域设置启动文档开发服务器（启用热重载）
- `pnpm start:es` - 以西班牙语区域设置启动文档开发服务器（启用热重载）
- `pnpm start:pt-br` - 以葡萄牙语（巴西）区域设置启动文档开发服务器（启用热重载）
- `pnpm build` - 为生产构建文档站点
- `pnpm write-translations` - 从文档中提取可翻译字符串
- `pnpm translate` - 使用 AI 翻译文档文件（参见[翻译工作流程](translation-workflow)）
- `pnpm lint` - 对文档源文件运行 ESLint

开发服务器（`start:*`）提供快速开发的热模块替换。默认端口是 3000。

### 生产脚本 {/* #production-scripts */}
- `pnpm build-local` - 构建并准备本地生产环境（包括预检查，将静态文件复制到独立目录）
- `pnpm start-local` - 在本地启动生产服务器（端口 8666，包括预检查）。**注意：** 先运行 `pnpm build-local`。使用 `--require ./scripts/peer-ip.cjs` 启动独立服务器。
- `pnpm start` - 使用相同的 peer-ip 预加载启动生产服务器（端口 9666）。Docker 使用 `docker-entrypoint.sh` 来加载相同脚本。
...
- `pnpm cron:dev` - 在开发模式下启动仅 cron 服务并启用文件监视（端口 8667）。通常在使用 `pnpm dev` 时不需要，因为它已经启动了 cron。
- `pnpm cron:start-local` - 在本地启动 cron 服务进行测试（端口 8667）

### 测试脚本 {/* #test-scripts */}
- `pnpm generate-test-data` - 生成测试备份数据（需要 --servers=N 参数）
- `pnpm validate-csv-export` - 验证 CSV 导出功能
- `pnpm test-entrypoint` - 在本地开发中测试 Docker 入口点脚本（参见[测试脚本](test-scripts)）
- `pnpm take-screenshots` - 为文档截取屏幕截图（参见[文档工具](documentation-tools)）

逾期检查、cron 健康检查和 SMTP 测试通过正在运行的应用程序和 `curl` 进行（参见[测试脚本](test-scripts)）；旧的独立 `pnpm` 辅助工具已被移除。
