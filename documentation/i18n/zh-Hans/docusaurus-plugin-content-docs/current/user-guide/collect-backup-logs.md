...
- **duplistatus** 数据库中已存在的任何日志都将被跳过。
- 新数据将被处理并存储在本地数据库中，包括每个备份日志中报告的 Duplicati 版本。[仪表板版本](dashboard.md#duplicati-server-version) 取自最新存储的日志 — **duplistatus** 不会读取服务器上当前正在运行的版本。在升级 Duplicati 后，收集或等待新的备份，以便仪表板可以显示新版本。
- 使用的 URL（带有检测到的协议）将被存储或更新在本地数据库中。
- 如果选择了下载选项，它将下载收集到的 JSON 数据，无论是否从 Duplicati 服务器接收到任何数据 — 即使日志验证失败或无法导入到数据库中。文件名将采用以下格式：`[serverName]_collected_[Timestamp].json`。时间戳使用 ISO 8601 日期格式（YYYY-MM-DDTHH:MM:SS）。
- 仪表板将更新以反映新信息。
...
- 检查 **duplistatus** 是否可以连接到 Duplicati 服务器。一个常见的问题是 DNS 名称解析（系统无法通过其主机名找到服务器）。在[故障排除部分](troubleshooting.md#collect-backup-logs-not-working)中了解更多信息。
- 确保您提供的密码是正确的。
- 在 Duplicati 2.4+ 上，收集时从 Duplicati 服务器设置中读取 machine-id，当 systeminfo 选项默认为空时。
