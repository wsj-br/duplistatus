...
设置页面提供了一个统一的界面，用于配置 **duplistatus** 的所有方面。您可以通过单击 [应用程序工具栏](../overview.md#application-toolbar) 中的 <IconButton icon="lucide:settings" /> **设置** 按钮来访问它。请注意，普通用户将看到一个简化的菜单，选项比管理员少。
      ...
      <ul>
        <li>
          ...
          <ul>
            ...
            <li><a href="daily-summary-settings.md">每日摘要</a>：可选的每日快照，替换发送给默认收件人的电子邮件（其他目标继续）</li>
        ...
        <li>
          ...
          <ul>
            ...
            <li><a href="display-settings.md">显示设置</a>：配置主题、图表时间范围、图表样式、格式区域、自动刷新间隔、卡片排序顺序和周开始日期</li>
      ...
      <ul>
        ...
        <li>
          ...
          <ul>
            ...
            <li><a href="display-settings.md">显示</a>：配置主题、图表时间范围、图表样式、格式区域、自动刷新间隔、卡片排序顺序和周开始日期</li>
            <li><a href="duplicati-versions.md">Duplicati 版本</a>：查看缓存的 Duplicati 发布版本（计划更改仅限管理员操作）</li>
...
当配置无效时，[备份通知](backup-notifications-settings.md) 选项卡中的相应复选框将变为灰色并禁用。有关详细信息，请参阅 [NTFY 设置](ntfy-settings.md) 和 [电子邮件设置](email-settings.md) 页面。

<br/>

:::important
绿色图标并不一定意味着通知功能正常工作。在依赖通知之前，请始终使用可用的测试功能确认您的通知正在正常工作。
:::
