...
当**每日摘要**启用时，默认电子邮件收件人将收不到电子邮件。此页面上的其他电子邮件目标仍将接收匹配的事件。此页面上的设置将被保留，并在每日摘要关闭时重新激活。请参阅[每日摘要](daily-summary-settings.md)。
...
## 配置单次备份通知设置 {/* #configure-per-backup-notification-settings */}
...
### 服务器级默认设置 {/* #server-level-defaults */}
...
### 单次备份配置 {/* #per-backup-configuration */}
...
- 当配置时，通知将发送到全局设置和其他目标。
- 其他目标的通知事件设置与主通知事件设置独立。
- 如果其他目标设置为**关闭**，则不会向这些目标发送通知，但主通知将根据主要设置正常工作。
- **过期**警报对于其他通知事件过滤器计为**警告**：当事件为**所有**或**警告**时发送，当事件为**错误**或**关闭**时不发送。相同的过滤器适用于其他NTFY主题。
- 当备份继承服务器默认值时，任何对服务器默认值的更改都将自动应用于该备份（除非已被覆盖）。
- 当[每日摘要](daily-summary-settings.md)启用时，其他电子邮件目标仍将接收匹配的事件；仅默认电子邮件收件人被抑制。
