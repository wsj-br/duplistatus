...
页面顶部有一个**模板语言**选择器，可让您选择默认模板的语言（英语、德语、法语、西班牙语、葡萄牙语、印地语和简体中文）。更改语言会更新默认的区域设置，但现有的自定义模板会保持当前文本，直到您更新它们或使用其中一个重置按钮。
...
| `{log_text}`           | 仅警告和错误日志行（非完整信息日志）。NTFY 使用简短摘要，可能会截断。 | 成功，警告 |
