...
被拒绝的请求返回 HTTP 403 (`IP_NOT_ALLOWED` on API paths)。它们不会被写入审计日志。一条速率限制的 `console.warn` 行被发送到应用程序 stdout (例如 `docker logs`) — 每个客户端 IP 和表面 (admin, external, or probe) 每分钟最多一条日志，每小时最多十条 — 这样扫描仪就无法淹没日志。
...
当启用时，`/api/upload`、`/api/summary` 和 `/api/lastbackup*` 仅接受列出的 CIDR。

`/api/health` 和 `/api/ping` 不仅在外部列表上 (仪表板 ping 来自 admin UI IP)。当**任一** 允许列表启用时，这些探针接受回环 (`127.0.0.1`、`::1`) 和来自 **admin 或 external** 列表的 CIDR。未列出的 IP 收到 HTTP 403。当两个列表都关闭时，探针保持公开。

非回环探针请求也受到速率限制 (HTTP 429, `PROBE_RATE_LIMITED`)：`/api/ping` 60/分钟和 600/小时；`/api/health` 30/分钟和 120/小时。容器内的 Docker 检查命中 localhost 并且永远不会被限制。应用程序级别的限制不会阻止体积连接洪水；将其放在反向代理上。
