...
启用后，页面、登录、CSRF和会话API仅接受列出的CIDR。使用**添加**添加条目；当前**允许列表 IP**在列表中时，标记为**当前IP**。**添加当前IP**和**最近的管理员登录IP**（来自审计日志）提供快速建议。除非当前IP已包含在内（或您从回环连接），否则无法启用此列表。可以通过以下方式恢复锁定：
...
此列表是不需要API密钥时使用的保护措施。像管理员列表一样添加CIDR作为芯片。审计日志中的**最近上传源IP**提供快速添加建议。
