...
[NTFY](https://github.com/binwiederhier/ntfy) 是一个简单的通知服务，可以将推送通知发送到您的手机或桌面。本节允许您设置通知服务器连接和身份验证。
...
| **NTFY 访问令牌** | 用于经过身份验证的 NTFY 服务器的可选访问令牌。如果您的服务器不需要身份验证，请留空此字段。               |
...
| <IconButton icon="lucide:qr-code" label="配置设备"/>          | 显示一个二维码，允许您快速配置移动设备或桌面以接收 NTFY 通知。 |

## 设备配置 {/* #device-configuration */}

在配置设备之前，您应该在设备上安装 NTFY 应用程序（[请参阅此处](https://ntfy.sh/)）。单击 <IconButton icon="lucide:qr-code" label="配置设备"/> 按钮，或右键单击应用程序工具栏中的 <SvgButton svgFilename="ntfy.svg" /> 图标，将显示一个二维码。扫描此二维码将自动配置您的设备，使其具有正确的 NTFY 主题以接收通知。
...
:::caution
如果您在没有访问令牌的情况下使用公共 **ntfy.sh** 服务器，任何知道您的主题名称的人都可以查看您的
通知。 
 
为了提供一定的隐私，系统会生成一个随机的 12 个字符的主题，提供超过
3 千兆（3,000,000,000,000,000,000,000）种可能的组合，使其难以猜测。
...
⚠️ **您需要自行保护您的 NTFY 主题。请自行决定是否使用此服务。**
:::
