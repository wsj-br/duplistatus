...
**要求外部 API 使用 API 密钥**开关默认处于关闭状态。在关闭期间，允许没有密钥的请求。如果客户端仍然发送密钥，有效且作用域匹配的密钥将被接受和记录；无效、已禁用、已过期或作用域错误的密钥将被忽略，并且请求仍然被允许。当您开启该开关时，四个外部数据 API 在没有有效密钥的情况下将返回 `401`（并拒绝错误的密钥）。请先启用至少一个上传密钥和一个读取密钥，否则 Duplicati 上传和主页小部件将停止工作。更改会自动保存。

## 外部 API 保护 {/* #external-api-protection */}

同一页面可以要求公共上传和读取 API 使用 API 密钥，并为 `/api/upload` 配置最大请求体大小（默认 5 MB）和基于 IP 的速率限制。即使密钥是可选的，大小和速率限制也适用，并且是防范泛洪攻击的主要防御手段。开关和限制字段会自动保存；没有单独的保存按钮。
