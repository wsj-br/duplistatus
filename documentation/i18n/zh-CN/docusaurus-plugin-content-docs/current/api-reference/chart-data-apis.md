...
- **方法**：GET
- **描述**：获取聚合图表数据，支持可选的时间范围筛选。
- **查询参数**：
  - `startDate`（可选）：筛选起始日期（ISO 格式）
  - `endDate`（可选）：筛选结束日期（ISO 格式）
- **响应**：
  ```json
  [
    {
...
- **错误响应**：
  ...
  - `500`：获取图表数据时服务器错误
- **说明**：
  - 支持使用 startDate 和 endDate 参数进行时间范围筛选
  - 处理前验证日期格式
  - 返回所有服务器的聚合数据
...
- **方法**：GET
- **描述**：获取特定服务器和备份的图表数据，支持可选的时间范围筛选。
- **参数**：
  - `serverId`：服务器标识符
