...
- pnpm (see `engines.pnpm` / `packageManager` in `package.json`)
- SQLite3
- Inkscape (for documentation SVG translation aur PNG export; required only if you run `translate` or `translate:svg`)
...
### 4. Install Node.js aur pnpm: {#4-install-nodejs-and-pnpm}
...
- Use random passwords for the `ADMIN_PASSWORD` aur `USER_PASSWORD`; these passwords will be used in the `pnpm take-screenshots` script.
