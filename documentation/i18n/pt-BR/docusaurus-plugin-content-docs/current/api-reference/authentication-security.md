---
translation_last_updated: '2026-05-06T23:19:20.487Z'
...
- **Autenticação**: Exige sessão válida e token CSRF (mas nenhum usuário logado)
- **Corpo da Requisição**:
...
- **Respostas de erro**: Incluem `error` e `errorCode` para tradução no lado do cliente.
  ...
  - `500`: Erro interno do servidor — `errorCode: "INTERNAL_ERROR"`
- **Notas**:
  - O cookie de sessão é removido na resposta
...
- **Notas**:
  ...
  - Se a flag `mustChangePassword` estiver definida, a verificação da senha atual é ignorada
  - Após alteração bem-sucedida da senha, a flag `mustChangePassword` é removida
...
- **Respostas de erro**: Incluem `error` e `errorCode` para tradução no lado do cliente.
  - `500`: Falha ao recuperar política de senhas — `errorCode: "POLICY_RETRIEVE_FAILED"`
- **Notas**:
  - Endpoint público, sem autenticação necessária
  - Utilizado por componentes frontend para exibir os requisitos de senha e validar senhas antes do envio
