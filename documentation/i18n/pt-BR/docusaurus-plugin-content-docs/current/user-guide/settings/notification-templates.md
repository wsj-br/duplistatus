...
| **Resumo Diário**  | Modelo de e-mail Markdown para o snapshot diário opcional. |

<br/>

## Idioma do Modelo {/* #template-language */}

Um seletor de **Idioma do Modelo** na parte superior da página permite escolher o idioma para modelos padrão (Inglês, Alemão, Francês, Espanhol, Português, Hindi e Chinês Simplificado). Alterar o idioma atualiza a localidade para padrões, mas modelos personalizados existentes mantêm seu texto atual até que você os atualize ou use um dos botões de redefinição.
...
| <IconButton icon="lucide:send" label="Enviar Notificação de Teste"/>     | Verifica o modelo após atualizá-lo. As variáveis serão substituídas pelos seus nomes para o teste. Para notificações por e-mail, o título do modelo se torna a linha de assunto do e-mail. Não disponível na guia Resumo Diário. |
...
Os corpos de e-mail são Markdown. Títulos, listas, links e tabelas são suportados. Valores de espaço reservado são inseridos como texto escapado e não podem introduzir Markdown ou HTML. HTML bruto incorporado anteriormente em modelos personalizados agora é escapado. NTFY recebe o mesmo Markdown, exceto tabelas GFM: a linha de cabeçalho é omitida e as linhas do corpo são enviadas como texto simples, para qualquer layout de coluna.

Os corpos padrão de Sucesso, Aviso/Erro e Atrasado usam o mesmo estilo Markdown que o Resumo Diário: um título, valores em negrito e uma tabela de visão geral. Padrões não modificados armazenados são atualizados ao carregar; modelos personalizados são mantidos.
...
| `{file_size}`          | Tamanho total dos arquivos de backup.                  | Sucesso, Aviso |
...
| `{log_text}`           | Linhas de log de aviso e erro apenas (não logs de Informação completos). NTFY usa um resumo curto e pode truncar. | Sucesso, Aviso |
...
| `{overdue_tolerance}`  | Configuração de tolerância para atraso.                      | Atrasado          |
...
| `{success_count}` / `{warning_count}` / `{error_count}` / `{fatal_count}` / `{unknown_count}` / `{no_report_count}` | Balde de status mutuamente exclusivos |
...
| `{duplistatus_link}` | Link para o painel duplistatus (omitido quando nenhum URL público está configurado). Prefira isso a links Markdown construídos manualmente. |
| `{duplistatus_url}` | Mesmo URL como texto simples (vazio quando nenhum URL público está configurado). |
...
O assunto do e-mail de Resumo Diário padrão é:
...
Use **Visualização** para renderizar o assunto do e-mail, HTML e texto simples sem enviar. Visualizações de Sucesso, Aviso/Erro e Atrasado também incluem o payload Markdown do NTFY. A visualização é aberta em uma caixa de diálogo. Os botões E-mail HTML / Texto simples / NTFY ficam acima do assunto. O E-mail HTML segue o tema claro ou escuro atual.
