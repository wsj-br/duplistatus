...
| **Frequência de Notificações**      | Com que frequência enviar notificações de atraso: <br/> **Uma vez`: Send **just one** notification when the backup becomes overdue. <br/> `Todos os dias`: Send **daily** notifications while overdue (default). <br/> `Toda semana`: Send **weekly** notifications while overdue. <br/> `Todo mês**: Envie notificações **mensais** enquanto estiver atrasado. <br/> Um canal com falha é registrado, e a janela de frequência ainda se aplica quando outro canal foi entregue. Os alertas se repetem em cada verificação de monitoramento apenas quando cada canal habilitado falha. |
