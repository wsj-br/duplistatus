...
O log de auditoria fornece um registro abrangente de todas as mudanças no sistema e ações dos usuários em **duplistatus**. Isso ajuda a rastrear mudanças de configuração, atividades dos usuários e operações do sistema para fins de segurança e solução de problemas.
...
Clique no ícone de olho <IconButton icon="lucide:eye" /> ao lado de qualquer entrada de log para ver informações detalhadas, incluindo:
- Timestamp completo
...
:::note
As exportações incluem apenas os logs atualmente visíveis com base em seus filtros ativos. Para exportar todos os logs, limpe todos os filtros primeiro.
:::
