...
duplistatus é um poderoso painel de monitoramento projetado especificamente para sistemas de backup do Duplicati. Ele fornece:

- Monitoramento centralizado de vários servidores Duplicati a partir de uma única interface
- Rastreamento de status em tempo real de todos os backups
...
- Suporte a múltiplos idiomas (Inglês, Francês, Alemão, Espanhol, Português do Brasil, Hindi e Chinês Simplificado).
...
duplistatus fornece um painel intuitivo para monitorar as operações de backup do Duplicati em toda a sua infraestrutura.

![Visão Geral do Painel](../assets/screen-main-dashboard-card-mode.png)

A interface do usuário é organizada em várias seções-chave para proporcionar uma experiência de monitoramento clara e abrangente:
...
A barra de ferramentas do aplicativo fornece acesso conveniente a funções e configurações principais, organizadas para um fluxo de trabalho eficiente.
...
| <IconButton icon="lucide:user" label="username" />                                                                                               | Mostrar o usuário conectado, tipo de usuário (`Admin`, `User`), clique para menu do usuário (inclui seleção de idioma). Veja mais em [Gerenciamento de Usuários](settings/user-management-settings.md)               |
| <IconButton icon="lucide:book-open-text" href="overview" /> &nbsp; Guia do Usuário                                                                    | Abra o [Guia do Usuário](overview.md) para a seção relevante à página que você está visualizando atualmente. A dica de ferramenta mostra "Ajuda para [Nome da Página]" para indicar qual documentação será aberta. |

### Menu do Usuário {/* #user-menu */}

Clicar no botão do usuário abre um menu suspenso com opções específicas do usuário. As opções do menu diferem dependendo de você estar logado como administrador ou usuário regular. Ambos os papéis podem alterar o idioma da interface por meio do submenu **Idioma**. O idioma selecionado é salvo por usuário neste navegador (não como uma configuração global do sistema), então diferentes contas podem manter diferentes idiomas. Idiomas suportados: inglês, francês, alemão, espanhol, português do Brasil, hindi e chinês simplificado.
...
2. Coletar logs de backup iniciais – use o recurso [Coletar Logs de Backup](collect-backup-logs.md) para preencher o banco de dados com dados históricos de backup de todos os seus servidores Duplicati. Isso também atualiza automaticamente os intervalos de monitoramento de backup com base na configuração de cada servidor.
3. Configure as definições do servidor – configure aliases e notas do servidor em [Configurações → Servidor](settings/server-settings.md) para tornar seu painel mais informativo.
4. Configure as definições do NTFY – configure notificações via NTFY em [Configurações → NTFY](settings/ntfy-settings.md).
...
7. Opcionalmente restrinja o acesso – crie [chaves de API](settings/api-keys-settings.md) e/ou [listas de permissão de IP](settings/ip-allowlist-settings.md) se você quiser proteger `/api/upload` e a interface de administração. Ambos estão desativados por padrão.
