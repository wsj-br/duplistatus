---
translation_last_updated: '2026-01-31T00:51:29.376Z'
...
- Cálculos de data/hora para status atrasado

Útil para depuração de sistemas de detecção e notificação de backup atrasado.

## Validar exportação de CSV {#validate-csv-export}

```bash
pnpm validate-csv-export
```

Este script valida a funcionalidade de exportação de CSV. Ele:
- Testa a geração de exportação de CSV
- Verifica o formato e estrutura dos dados
...
Este script bloqueia temporariamente o acesso à rede de saída para o servidor NTFY (`ntfy.sh`) para testar o mecanismo de repetição de notificações. Ele:
...
O projeto inclui scripts para testar migrações de banco de dados de versões antigas para a versão atual. Estes scripts garantem que as migrações de banco de dados funcionem corretamente e preservem a integridade dos dados.

### Gerar Dados de Testar de Migração {#generate-migration-test-data}
...
2. **Para cada versão** (v0.4.0, v0.5.0, v0.6.1, 0.7.27, 0.8.21):
   ...
   - Libera arquivos WAL e salva o esquema de banco de dados
...
**Configuração:**
- Número de servidores: Defina via variável `SERVERS` (padrão: 3)
...
1. **Para cada versão** (v0.4.0, v0.5.0, v0.6.1, 0.7.27, 0.8.21):
   ...
   - Verifica as tabelas e colunas obrigatórias
...
**Saída:**
- Resultados de teste com código de cores (verde para aprovado, vermelho para falha)
...
- Todos os tabelas obrigatórias existem: `servers`, `backups`, `configurations`, `users`, `sessions`, `audit_log`, `db_version`
- Colunas obrigatórias existem em cada tabela
...
>[!NOTE]
> Este script utiliza internamente o script de teste de migração TypeScript (`test-migration.ts`). O script de teste valida a estrutura do banco de dados após a migração e garante a integridade dos dados.

## Definir Configuração de Teste SMTP {#set-smtp-test-configuration}

```bash
pnpm set-smtp-test-config <connectionType>
```

Este script define a configuração de testar SMTP a partir de variáveis de ambiente. Ele aceita um parâmetro `connectionType` (`plain`, `starttls` ou `ssl`) e lê variáveis de ambiente correspondentes com prefixos (`PLAIN_`, `STARTTLS_`, `SSL_`) para atualizar a configuração SMTP no banco de dados.
...
## Testar Compatibilidade Cruzada do Tipo de Conexão SMTP {#test-smtp-connection-type-cross-compatibility}
...
**Requisitos:**
- A aplicação deve estar em execução
- As variáveis de ambiente devem ser definidas para todos os três tipos de conexão
- O script valida a configuração sendo utilizada através de registros detalhados
...
**Saída:**
- Saída do console com uma tabela de resumo mostrando os resultados do teste
- Arquivo `smtp-test-results.json` com resultados detalhados do teste para cada combinação de configuração e tipo de conexão
...
1. **Sempre constrói uma versão nova**: Executa automaticamente `pnpm build-local` para criar uma compilação nova antes de testar (sem necessidade de compilar manualmente primeiro)
...
**Output:**
- Logs são escritos em `data/logs/application.log` (acessível pela aplicação)
...
**Casos de Uso:**
- Testar alterações de script de ponto de entrada localmente antes da implantação Docker
- Verificar rotação de logs e funcionalidade de logging
- Testar encerramento gracioso e tratamento de sinais
- Depurar comportamento do script de ponto de entrada em um ambiente local
