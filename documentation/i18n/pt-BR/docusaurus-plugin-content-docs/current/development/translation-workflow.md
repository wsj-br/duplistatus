...
Use IDs explícitos para que os links permaneçam estáveis em diferentes idiomas. Prefira a sintaxe de comentário MDX (`pnpm write-heading-ids` usa `--syntax mdx-comment`):

```markdown
## This is a heading {/* #this-is-a-heading */}
```

Coloque IDs em `h2` e abaixo. O Docusaurus `write-heading-ids` ignora `h1` (o título da página/barra lateral). `documentation/docusaurus.config.ts` também remove comentários de heading-id de títulos inferidos, porque a extração de metadados do Docusaurus ainda só remove comentários clássicos `{#id}`.
