---
translation_last_updated: '2026-01-31T00:51:30.666Z'
...
Etapas manuais são necessárias apenas se você tiver personalizado Modelos de notificação (versão 0.8.x alterou variáveis de modelo) ou Integrações de API externas que precisam ser atualizadas (versão 0.7.x alterou nomes de campos de API, versão 0.9.x requer autenticação).

## Visão Geral {#overview}

O duplistatus migra automaticamente o esquema do seu banco de dados ao fazer upgrade. O sistema:

1. Cria um backup de seu banco de dados antes de fazer alterações
2. Atualiza o esquema de banco de dados para a versão mais recente
...
### Se Você Está Executando a Versão 1.2.1 ou Posterior {#if-youre-running-version-121-or-later}
...
4. O arquivo de backup será baixado no seu computador com um nome de arquivo com timestamp
...
### Se Você Está Executando uma Versão Anterior a 1.2.1 {#if-youre-running-a-version-before-121}
...
2. Acesse a aba Containers e clique no seu container duplistatus.
...
##### Para Usuários Linux {#for-linux-users}
A forma mais fácil de restaurar é "enviar" o arquivo de backup de volta para o caminho de armazenamento interno do container.
...
##### Para Usuários Windows {#for-windows-users}
Se você está usando Docker Desktop, pode realizar a restauração através da GUI ou PowerShell.
...
4. **Verificação**: O sucesso da migração é registrado em log
...
1. **Entrar** com as credenciais padrão de admin:
...
- Esquema de banco de dados atualizado para v3.1
...
- Certifique-se de que o arquivo `.duplistatus.key` está com backup (com permissões 0400)
...
Consulte [Alterações Significativas da API](api-changes.md) para obter etapas detalhadas de migração da API.
...
- [ ] Alterada a senha padrão do Admin (0.9.x+)
- [ ] Criadas contas de Usuário para outros usuários (0.9.x+)
- [ ] Atualizadas as integrações de API externa com autenticação (0.9.x+)
...
4. Restaurar a partir de backup se necessário (consulte Reversão abaixo)
...
1. Revise [Alterações Significativas da API](api-changes.md) para atualizações de endpoints
...
:::warning
A reversão pode causar perda de dados se o esquema mais recente for incompatível com a versão anterior. Sempre certifique-se de ter um backup recente antes de tentar reverter.
:::

### Solução de Problemas da Sua Restauração / Reversão {#troubleshooting-your-restore--rollback}

Se a aplicação não iniciar ou seus dados não aparecerem após uma restauração ou reversão, verifique os seguintes problemas comuns:

#### 1. Permissões de Arquivo do Banco de Dados (Linux/Podman) {#1-database-file-permissions-linuxpodman}

Se você restaurou o arquivo como o usuário `root`, a aplicação dentro do contêiner pode não ter permissão para ler ou escrever nele.
...
* **O Sintoma:** A aplicação inicia mas aparenta estar "vazia" (como uma instalação nova).
* **A Solução:** Verifique o diretório `/app/data/`. Se seu arquivo for nomeado `duplistatus-backup-2024.db` ou tiver uma extensão `.sqlite`, a aplicação o ignorará. Use o comando `mv` ou a interface gráfica do Docker Desktop para renomeá-lo exatamente para `backups.db`.
...
## Versões do Schema do Banco de Dados {#database-schema-versions}

| Versão da Aplicação        | Versão do Schema | Principais Alterações                                        |
|----------------------------|----------------|----------------------------------------------------|
| 0.6.x e anteriores          | v1.0           | Schema inicial                                     |
| 0.7.x                      | v2.0, v3.0     | Configurações adicionadas, servidores renomeados   |
