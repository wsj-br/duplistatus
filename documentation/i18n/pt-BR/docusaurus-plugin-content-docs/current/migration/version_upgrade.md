...
### Se Você Está Usando a Versão 1.2.1 ou Superior {/* #if-youre-running-version-121-or-later */}
...
### Se Você Está Usando uma Versão Anterior à 1.2.1 {/* #if-youre-running-a-version-before-121 */}
...
##### Para Usuários de Linux {/* #for-linux-users */}
Se você está no Linux, não se preocupe em iniciar contêineres auxiliares. Você pode usar o comando nativo `cp` para extrair o banco de dados diretamente do contêiner em execução para o seu host.
...
##### Para Usuários de Windows {/* #for-windows-users */}
Se você está usando o Docker Desktop no Windows, tem duas maneiras simples de lidar com isso sem usar a linha de comando:

###### Opção A: Usar Docker Desktop (Mais Fácil) {/* #option-a-use-docker-desktop-easiest */}
1. Abra o Painel do Docker Desktop.
2. Vá para a aba Containers e clique no seu contêiner duplistatus.
...
Se você prefere o terminal, pode usar o PowerShell para copiar o arquivo para a sua Área de Trabalho:

```powershell
docker cp duplistatus:/app/data/backups.db $HOME\Desktop\duplistatus-backup.db
```

##### Se Você Usa Montagens Bind {/* #if-you-use-bind-mounts */}
Se você configurou originalmente seu contêiner usando uma montagem bind (por exemplo, você mapeou uma pasta local como `/opt/duplistatus` para o contêiner), você não precisa de comandos do Docker. Basta copiar o arquivo usando seu gerenciador de arquivos:
- Linux: `cp /path/to/your/folder/backups.db ~/backups.db`
- Windows: Basta copiar o arquivo no **Explorador de Arquivos** da pasta que você designou durante a configuração.

#### Restaurando Seus Dados {/* #restoring-your-data */}
Se você precisar restaurar seu banco de dados de um backup anterior, siga os passos abaixo com base no seu sistema operacional.

:::info[IMPORTANTE] 
Parar o container antes de restaurar o banco de dados para evitar corrupção de arquivos.
:::

##### Para Usuários de Linux {/* #for-linux-users-1 */}
...
##### Para Usuários de Windows {/* #for-windows-users-1 */}
Se você está usando o Docker Desktop, pode realizar a restauração via GUI ou PowerShell.

###### Opção A: Usar Docker Desktop (GUI) {/* #option-a-use-docker-desktop-gui */}
1. Certifique-se de que o contêiner duplistatus está em execução (o Docker Desktop requer que o contêiner esteja ativo para enviar arquivos via GUI).
...
###### Opção B: Usar PowerShell {/* #option-b-use-powershell-1 */}
...
##### Se Você Usa Montagens Bind {/* #if-you-use-bind-mounts-1 */}
Se você está usando uma pasta local mapeada para o contêiner, não precisa de comandos especiais.
...
### Atualizando para a Versão 0.9.x ou Posterior (Esquema v4.0) {/* #upgrading-to-version-09x-or-later-schema-v40 */}
...
### Dados Faltando Após a Migração {/* #data-missing-after-migration */}
...
### Solucionando Problemas de Sua Restauração / Reversão {/* #troubleshooting-your-restore--rollback */}
