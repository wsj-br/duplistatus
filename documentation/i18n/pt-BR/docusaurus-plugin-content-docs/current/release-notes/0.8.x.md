---
translation_last_updated: '2026-02-14T04:57:50.336Z'
...
- **Persistência automática de URL do servidor e senha** ao coletar backups
  - Os usuários podem atualizar a URL e a senha usando dois métodos:
     - Coletando backups novamente com valores atualizados
     - Modificando as configurações em `Configurações → Configurações do Servidor`

### Monitoramento de backups atrasados {#enhanced-overdue-monitoring}

- **Nova aba de configuração `Overdue Monitoring`** com configurações de intervalo compatíveis com servidor Duplicati
    - Suporte para intervalos personalizados (por exemplo, "1D12h")
    - Atualizações automáticas de intervalo atrasado a partir da Configuração do Duplicati durante a coleta de log de backup
