---
translation_last_updated: '2026-02-05T19:09:06.985Z'
...
- **Exportação CSV aprimorada**: Adicionada coluna "Last Backup Timestamp (DB)" ao arquivo CSV gerado pelo botão "Baixar CSV" nas configurações de Monitoramento de backups atrasados. Isso permite que os usuários comparem a data e hora do último backup real do banco de dados com a data e hora do "Último backup" configurada.
...
- **Correção de datas e horas do log de auditoria**: As datas e horas no visualizador de log de auditoria agora são exibidas corretamente no fuso horário do navegador do usuário em vez de GMT. As datas e horas do SQLite (armazenadas em UTC) agora são analisadas corretamente como UTC e convertidas para o fuso horário local do navegador para exibição.
- **Cálculo corrigido de "Próxima execução" e detecção de atrasos**: A coluna "Próxima execução" nas configurações de monitoramento de backups atrasados agora calcula e exibe com precisão a próxima data de backup esperada, levando em consideração tanto o agendamento de backup quanto a data e hora do último backup. Isso se aplica tanto a trabalhos de backup agendados quanto aos executados manualmente, resultando em detecção mais confiável de atrasos.
...
- **Adicionado**: Coluna "Data e hora do último backup (BD)" na exportação CSV de monitoramento de backups atrasados
- **Adicionado**: Dica de ferramenta para o texto "Próxima execução" nas configurações de monitoramento de backups atrasados
- **Adicionado**: Entradas individuais de log de auditoria para cada notificação e e-mail enviados
- **Adicionado**: Robustez aprimorada de log de auditoria para falhas de notificação
...
- **Adicionado**: Endpoint de API `GET /api/auth/admin-must-change-password` para verificar requisito de alteração de senha de admin
...
- **Alterado**: Formulários de alteração de senha não exigem mais confirmação ao mostrar senhas sem máscara.
