---
translation_last_updated: '2026-01-31T00:51:30.720Z'
...
Versão 0.9.x é uma versão principal que introduz um **Sistema Abrangente de Controle de Acesso de Usuários e Segurança**. Esta versão transforma o **duplistatus** de uma aplicação de acesso aberto em um sistema seguro, multi-usuário com controle de acesso baseado em funções, registro de auditoria abrangente e recursos de segurança de nível empresarial. Toda a funcionalidade existente agora está protegida por autenticação, garantindo que apenas usuários autorizados possam acessar e gerenciar o monitoramento de backup.
...
- **Conta de admin padrão** é criada automaticamente:
...
- **Endpoints de API agora exigem autenticação** - As integrações externas devem incluir cookies de sessão ou usar requisições autenticadas

:::warning
**Todos os usuários devem entrar após atualizar para a versão 0.9.x.** As credenciais padrão do admin são admin/Duplistatus09. Altere a senha imediatamente após o primeiro login.
:::

### Migração do Schema do Banco de Dados {#database-schema-migration}
...
### Controle de Acesso do Usuário e Sistema de Segurança {#user-access-control-security-system}
...
- **Funcionalidade "Lembrar de mim"** com persistência de Nome de usuário no localStorage
...
- **Tarefa cron de limpeza automática** (executada diariamente às 2 AM UTC)
- **API de limpeza manual** com suporte a dry-run
...
- **Persistência de estado da barra lateral específica do usuário** (preferência de recolhimento/expansão salva por usuário)
- **Seções de configurações agrupadas**:
  - **Notificações**: Notificações de backup, Monitoramento de backups atrasados, Modelos
...
- **Visibilidade de UI baseada em função** (recursos exclusivos de admin ocultos de usuários comuns)
- **Emblemas e indicadores de status** em toda a UI
...
1. **Migração automática do banco de dados**: A aplicação migrará automaticamente o esquema do seu banco de dados de v3.1 para v4.0
...
5. **Login de usuário obrigatório**: Todos os usuários devem entrar após a atualização
...
- **Configurar Retenção**: Defina um período apropriado de retenção de log de auditoria com base nos seus requisitos de conformidade
- **Backup da Chave Mestra**: Se estiver atualizando a partir da versão 0.8.x, certifique-se de que o arquivo `.duplistatus.key` está em backup
...
- Corrigidos problemas de persistência de sessão entre reinicializações do servidor
- Melhorado o tratamento de erros para falhas de autenticação
- Aprimorado o feedback de validação de Senha
- Corrigidos inconsistências de interface do usuário na Página de Configurações
- Melhorado o desempenho de filtragem do Log de Auditoria
...
- **Adicionado**: Ferramenta CLI de recuperação do Admin
...
- **Corrigido**: Várias inconsistências de interface do usuário
