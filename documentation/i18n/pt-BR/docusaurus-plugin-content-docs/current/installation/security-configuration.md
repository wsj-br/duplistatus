...
1. Manter a porta `9666` fora da internet pública: vincular o aplicativo ao localhost ou a uma rede privada.
2. Criar [chaves de API](#api-keys) e ativar **Exigir chaves de API para APIs externas**. Isso funciona sem um proxy reverso e é o primeiro passo mais fácil.
3. Disponibilizar o **duplistatus** por meio de um [proxy reverso com HTTPS](#https-with-a-reverse-proxy).
4. Adicionar o endereço de conexão do proxy a **Proxies confiáveis** (ou `IP_TRUSTED_PROXIES`) se você pretender usar listas de permissões.
5. Opcionalmente, ativar as [listas de permissões de IP](#ip-allowlist) de administrador e externas, usando o **IP detectado** e as sugestões de IP recentes para evitar o bloqueio do seu próprio acesso.
...
A diretiva `reverse_proxy` do Caddy define os cabeçalhos de IP do cliente para você. Você ainda precisa listar o endereço de conexão do proxy em **Proxies confiáveis** quando usar listas de permissões de IP (veja [abaixo](#trusted-proxies-for-ip-allowlists)).
...
Quando as [listas de permissões de IP](../user-guide/settings/ip-allowlist-settings.md) estiverem habilitadas, liste o proxy em **Proxies confiáveis** (ou defina `IP_TRUSTED_PROXIES`). O aplicativo respeita `X-Forwarded-For` e `X-Real-IP` apenas quando o endereço de conexão é um proxy confiável; caso contrário, ele os ignora.
