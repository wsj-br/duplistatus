---
translation_last_updated: '2026-05-06T23:21:40.843Z'
...
- **Corrigé** : Avertissements pnpm install — dépendance homologue webpack (allowedVersions) et sous-dépendances obsolètes (prebuild-install, whatwg-encoding) via allowedDeprecatedVersions dans le package.json racine.
- **Corrigé** : Avertissement useEffect exhaustive-deps dans la Visionneuse des journaux d'application (tableau de dépendances intentionnel ; ajout de eslint-disable avec commentaire).
...
- **Sécurité** : Remplacements pnpm pour minimatch (>=3.1.4) et serialize-javascript (>=7.0.3) afin de corriger des vulnérabilités de gravité élevée.
- **Modifié** : Documentation mise à jour avec les noms des scripts Docker (docker:up, docker:down, docker:clean, docker:devel).
- **Modifié** : Documentation et règles du projet mises à jour avec les versions actuelles des paquets provenant de package.json.
