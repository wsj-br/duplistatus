---
translation_last_updated: '2026-02-05T19:08:23.245Z'
...
- **Exportation CSV améliorée** : Ajout de la colonne « Horodatage de la dernière sauvegarde (BD) » au fichier CSV généré par le bouton « Télécharger CSV » dans les paramètres de surveillance des sauvegardes en retard. Cela permet aux utilisateurs de comparer l'horodatage réel de la dernière sauvegarde de la base de données avec l'horodatage configuré « Dernière sauvegarde ».
...
- **Horodatages du Journal d'Audit corrigés** : Les horodatages dans le Visualiseur de journal d'audit s'affichent désormais correctement dans le fuseau horaire du navigateur de l'utilisateur au lieu de GMT. Les horodatages SQLite (stockés en UTC) sont désormais correctement analysés en tant qu'UTC et convertis au fuseau horaire local du navigateur pour l'affichage.
- **Calcul de « Prochaine exécution » corrigé et détection des sauvegardes en retard** : La colonne « Prochaine exécution » dans les paramètres de Surveillance des sauvegardes en retard calcule et affiche désormais avec précision la date de sauvegarde attendue suivante, en tenant compte à la fois de la planification de sauvegarde et de l'horodatage de la dernière sauvegarde. Ceci s'applique aux tâches de sauvegarde planifiées et exécutées manuellement, ce qui permet une détection plus fiable des sauvegardes en retard.
...
- **Ajouté** : Système de valeurs par défaut et d'héritage au niveau du serveur pour les notifications de sauvegarde
- **Ajouté** : Fonctionnalité de test d'e-mail et de test de notification dans les paramètres de Notifications de sauvegarde
...
- **Ajouté** : Entrées de journal d'audit individuelles pour chaque notification et e-mail envoyés
...
- **Ajouté** : Point de terminaison API `GET /api/auth/admin-must-change-password` pour vérifier l'exigence de modification du mot de passe administrateur
...
- **Modifié** : Les formulaires de modification de mot de passe ne nécessitent plus de confirmation lors de l'affichage des mots de passe non masqués.
- **Modifié** : Les mots de passe sont effacés de manière sécurisée de la mémoire lorsque les formulaires de modification de mot de passe sont fermés.
- **Modifié** : Ajout d'un bouton d'affichage du mot de passe dans le formulaire Collecter les sauvegardes.
...
- **Corrigé** : Journal d'audit des échecs d'e-mail et NTFY
