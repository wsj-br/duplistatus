---
translation_last_updated: '2026-01-31T00:51:19.820Z'
...
- **Schema v2.0** (Application v0.7.x) : Ajout de colonnes manquantes et table de configurations
...
Version actuelle de l'application (v1.1.0) utilise **Schema v4.0** comme version de schéma de base de données la plus récente.
...
| `files_with_error`    | INTEGER | Fichiers contenant des erreurs       |
...
| Field                    | Type    | Description                                    |
|--------------------------|---------|------------------------------------------------|
| `size_of_examined_files` | INTEGER | Taille de fichiers examinés lors de la sauvegarde |
| `size_of_opened_files`   | INTEGER | Taille de fichiers ouverts pour la sauvegarde |
| `size_of_added_files`    | INTEGER | Taille de nouveaux fichiers ajoutés à la sauvegarde |
| `size_of_modified_files` | INTEGER | Taille de fichiers modifiés dans la sauvegarde |
...
#### Champs de Statistiques du Backend {#backend-statistics-fields}
...
| `is_admin`              | BOOLEAN NOT NULL     | Indique si l'utilisateur est Admin  |
| `must_change_password`  | BOOLEAN              | Indique si le changement de mot de passe est requis |
...
| `last_accessed`  | DATETIME          | Horodatage de dernière consultation                              |
...
Stocke la piste d'audit des Actions de l'Utilisateur et des événements du Système.
...
| `category`     | TEXT NOT NULL                     | Catégorie de l'action (par ex. « authentication », « settings », « backup ») |
| `target_type`  | TEXT                              | Type de cible (par ex. « server », « backup », « user »)          |
...
### Stockage de session basé sur une base de données {#database-backed-session-storage}

Les sessions sont stockées dans la base de données avec un mécanisme de secours en mémoire :
- **Stockage principal** : Table de sessions sauvegardée en base de données
- **Mécanisme de secours** : Stockage en mémoire (support hérité ou cas d'erreur)
...
- **Clés étrangères** : Références de Serveur dans le tableau des Sauvegardes, références d'Utilisateur dans les sessions et audit_log
...
### Obtenir un résumé global {#get-overall-summary}
