...
3. `start.duplistatus` : Démarre le conteneur en mode sans privilèges.
...
- **Prise en charge de secours** : Retombe sur l'analyse de `/etc/resolv.conf` sur les systèmes non systemd
...
- **Fonctionne avec** :
  ...
  - Configurations réseau standards
...
4. Arrêtez avec `./stop.duplistatus` lorsque vous avez terminé
