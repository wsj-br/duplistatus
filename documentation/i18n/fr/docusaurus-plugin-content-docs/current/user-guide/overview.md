...
- Métriques et visualisation complètes des performances de sauvegarde
...
L'interface utilisateur est organisée en plusieurs sections clés pour fournir une expérience de surveillance claire et complète :
...
3. Aperçu des serveurs : [disposition en cartes](dashboard.md#cards-layout) ou [disposition en tableau](dashboard.md#table-layout) montrant le dernier état de toutes les sauvegardes, y compris la [version du serveur Duplicati](dashboard.md#duplicati-server-version) du dernier journal de sauvegarde reçu
...
La barre d'outils de l'application offre un accès pratique aux fonctions et paramètres clés, organisés pour un flux de travail efficace.
...
| <IconButton icon="lucide:rotate-ccw" /> &nbsp; Actualiser l'écran                                                                                    | Exécuter un rafraîchissement manuel immédiat de tous les données                                                                                                                                     |
| <IconButton label="Auto-actualisation" />                                                                                                              | Activer ou désactiver la fonctionnalité de rafraîchissement automatique. Configurer dans [Paramètres d'affichage](settings/display-settings.md) <br/> _Clic droit_ pour ouvrir la page Paramètres d'affichage                         |
...
| <SvgButton svgFilename="duplicati_logo.svg" href="duplicati-configuration" /> &nbsp; [Configuration de Duplicati](duplicati-configuration.md)       | Ouvrir l'interface web du serveur Duplicati sélectionné <br/> _Clic droit_ pour ouvrir l'interface utilisateur legacy de Duplicati (`/ngax`) dans un nouvel onglet                                                              |
...
| <IconButton icon="lucide:user" label="nom d'utilisateur" />                                                                                               | Afficher l'utilisateur connecté, le type d'utilisateur (`Admin`, `User`), cliquer pour le menu utilisateur (inclut la sélection de la langue). Voir plus dans [Gestion des utilisateurs](settings/user-management-settings.md)               |
| <IconButton icon="lucide:book-open-text" href="overview" /> &nbsp; Guide de l'utilisateur                                                                    | Ouvrir le [Guide de l'utilisateur](overview.md) pour la section pertinente de la page que vous consultez actuellement. L'info-bulle affiche "Aide pour [Nom de la page]" pour indiquer quelle documentation sera ouverte. |

### Menu utilisateur {/* #user-menu */}

En cliquant sur le bouton utilisateur, un menu déroulant s'ouvre avec des options spécifiques à l'utilisateur. Les options du menu diffèrent selon que vous êtes connecté en tant qu'administrateur ou qu'un utilisateur régulier. Les deux rôles peuvent changer la langue de l'interface via le sous-menu **Langue**. La langue sélectionnée est enregistrée par utilisateur sur ce navigateur (et non comme paramètre système global), de sorte que différents comptes peuvent conserver différentes langues. Langues prises en charge : anglais, français, allemand, espagnol, portugais brésilien, hindi et chinois simplifié.
...
2. Collectez les journaux de sauvegarde initiaux – utilisez la fonctionnalité [Collecter les journaux de sauvegarde](collect-backup-logs.md) pour peupler la base de données avec des données historiques de sauvegarde de tous vos serveurs Duplicati. Cela met également automatiquement à jour les intervalles de surveillance des sauvegardes en fonction de la configuration de chaque serveur.
3. Configurez les paramètres du serveur – configurez les alias et les notes des serveurs dans [Paramètres → Serveur](settings/server-settings.md) pour rendre votre tableau de bord plus informatif.
...
7. Optionnellement restreignez l'accès – créez des [clés API](settings/api-keys-settings.md) et/ou des [listes d'adresses IP autorisées](settings/ip-allowlist-settings.md) si vous souhaitez protéger `/api/upload` et l'interface d'administration. Les deux sont désactivés par défaut.
