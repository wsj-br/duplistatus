...
| **Fréquence des notifications**      | Fréquence d'envoi des notifications de retard : <br/> **Une fois`: Send **just one** notification when the backup becomes overdue. <br/> `Tous les jours`: Send **daily** notifications while overdue (default). <br/> `Toutes les semaines`: Send **weekly** notifications while overdue. <br/> `Tous les mois** : Envoyer des notifications **mensuelles** tant que le retard persiste. <br/> L'échec d'une chaîne est journalisé, et la fenêtre de fréquence s'applique toujours quand une autre chaîne a été délivrée. Les alertes se répètent à chaque vérification de surveillance uniquement quand toutes les chaînes activées sont en échec. |
