...
Ce document contient un exemple sur la charge utile JSON envoyée par Duplicati lors de l'utilisation de `--send-http-json-urls` (ou de l'ancienne option `--send-http-url` avec `--send-http-result-output-format=Json`). Il illustre la structure et les champs que duplistatus reçoit et persiste. Certains champs peuvent être effacés ou masqués dans cet exemple.
