...
Pour que cette application fonctionne correctement, chaque serveur Duplicati doit être configuré pour envoyer des rapports HTTP pour chaque sauvegarde au serveur **duplistatus**.

Appliquez cette configuration à chaque serveur Duplicati :

1. **Configurer les rapports de résultats de sauvegarde** : Sur la page de configuration de Duplicati, sélectionnez `Settings` et, dans la section `Default Options`, incluez les options suivantes.
...
Voir la documentation des [notifications HTTP](https://docs.duplicati.com/monitoring-and-notifications/sending-reports-via-email/sending-http-notifications) de Duplicati pour la référence des options.
...
Duplicati ne peut pas définir des en-têtes HTTP personnalisés. Le paramètre de requête est le moyen pris en charge pour envoyer la clé. Les journaux d'accès du reverse-proxy contiendront le secret, donc restreignez qui peut lire ces journaux.
...
Si votre serveur Duplicati est antérieur à 2.0.9.106, utilisez l'option URL legacy et définissez le format de résultat sur JSON :
...
- Si vous omettez `--send-http-log-level=Information`, aucun message de journal ne sera envoyé à **duplistatus**, seulement des statistiques. Cela empêchera la **liste** des versions disponibles de fonctionner.
- La valeur par défaut de Duplicati est `--send-http-max-log-lines=100`. La valeur recommandée est `500`. Duplicati conserve les **premières** N lignes de journal. Les lignes utilisées pour la liste des versions disponibles (`Backups to consider`) se trouvent généralement dans ces premières centaines de lignes ; `100` est souvent trop peu.
- `--send-http-max-log-lines=0` signifie illimité. Utilisez cela uniquement si la liste des versions est toujours manquante et que vous **n'êtes pas** également en train d'envoyer des rapports à [Duplicati Monitoring](https://www.duplicati-monitoring.com/). Des journaux illimités peuvent faire en sorte que ce service renvoie HTTP 500 sur de grandes tâches.
- Le **nombre** de versions disponibles provient toujours des statistiques JSON (`BackupListCount`) même lorsque la liste détaillée des horodatages est manquante. Si l'icône de la liste est grisée, augmentez le plafond (ou utilisez `0` lorsque vous ne rapportez qu'à **duplistatus**).

:::tip
Après avoir configuré le serveur **duplistatus**, collectez les journaux de sauvegarde pour tous vos serveurs Duplicati à l'aide de [Collecter les journaux de sauvegarde](../user-guide/collect-backup-logs.md).
:::

### Envoi de rapports à duplistatus et Duplicati Monitoring {/* #reporting-to-duplistatus-and-duplicati-monitoring */}

Vous pouvez envoyer des rapports depuis le **même** serveur Duplicati à **duplistatus** et à [Duplicati Monitoring](https://www.duplicati-monitoring.com/) en même temps. **duplistatus** doit recevoir du JSON. Duplicati Monitoring attend des rapports encodés en formulaire. Ne pointez pas `--send-http-form-urls` sur `/api/upload`.
...
- Utilisez `500`, pas `0`. Si Duplicati Monitoring retourne toujours HTTP 500 pour les tâches volumineuses, baissez le seuil (ou omettez `Information`) en sachant que la **liste** des versions peut être manquante. Si la liste est manquante mais que Monitoring fonctionne, augmentez le seuil. Sinon, signalez uniquement à **duplistatus** pour ces tâches.

:::caution
Si une cible HTTP échoue (indisponibilité ou HTTP 500), Duplicati peut ne pas envoyer les rapports restants. Les URLs de formulaire sont envoyées en premier, puis les URLs JSON. Une indisponibilité ou un 500 de Duplicati Monitoring peut donc bloquer le rapport JSON à **duplistatus**.
:::

[Collecter les journaux de sauvegarde](../user-guide/collect-backup-logs.md) ne dépend pas de l'envoi HTTP. Utilisez-le pour compléter une exécution qui n'a pas été reçue.

### Duplicati et duplistatus sur le même hôte {/* #duplicati-and-duplistatus-on-the-same-host */}

L'URL de téléchargement doit être accessible **à partir du processus Duplicati**, pas depuis votre navigateur.
...
2. **Optionnel - Autoriser l'accès à l'interface utilisateur à distance :** Si vous souhaitez accéder à l'interface web de Duplicati directement depuis les liens du tableau de bord **duplistatus**, connectez-vous à [l'interface utilisateur de Duplicati](https://docs.duplicati.com/getting-started/set-up-a-backup-in-the-ui), sélectionnez `Settings`, et autorisez l'accès à distance, y compris une liste de noms d'hôtes (ou utilisez `*`). Si vous passez cette étape, **duplistatus** recevra toujours les rapports de sauvegarde, mais les liens directs vers l'interface utilisateur de Duplicati ne fonctionneront pas.
...
(par exemple, VPN, LAN privée ou règles de pare-feu). Exposer l'interface Duplicati sur Internet sans mesures de sécurité appropriées pourrait entraîner un accès non autorisé.
