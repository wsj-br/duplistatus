...
Pour les déploiements en production, il est recommandé de servir **duplistatus** sur HTTPS en utilisant un proxy inverse. Cette section fournit des exemples de configuration pour les solutions de proxy inverse populaires.
...
- Nom de domaine pointant vers votre Serveurs
- Nginx installé sur votre Système
- Certbot installé pour votre Système
...
```bash
# Activer le site {#enable-the-site}
sudo ln -s /etc/nginx/sites-available/duplistatus /etc/nginx/sites-enabled/
sudo nginx -t
sudo systemctl reload nginx

# Obtenir le certificat SSL {#obtain-ssl-certificate}
...
Suivez le [guide d'installation officiel](https://caddyserver.com/docs/install) pour votre Système.
...
Ou utilisez-le comme un Système service :

```bash
sudo caddy Démarrer --config Caddyfile
```

Caddy obtiendra et gérera automatiquement les certificats SSL de Let's Encrypt.
...
:::info[IMPORTANT]
Après avoir configuré HTTPS, n'oubliez pas de mettre à jour la configuration de votre Serveurs Duplicati pour utiliser l'URL HTTPS :
...
- Assurez-vous que l'enregistrement DNS A de votre domaine pointe vers l'Adresse IP de votre Serveurs
