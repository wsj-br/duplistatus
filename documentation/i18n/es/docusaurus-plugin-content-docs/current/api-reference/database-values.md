# Informe de ejemplo de copia de seguridad (valores de la base de datos) {/* #sample-backup-report-database-values */}

Este documento contiene una muestra de la carga útil JSON enviada por Duplicati al usar `--send-http-json-urls` (o la opción heredada `--send-http-url` con `--send-http-result-output-format=Json`). Ilustra la estructura y los campos que duplistatus recibe y persiste. Algunos campos pueden haberse borrado u ocultado en este ejemplo.
