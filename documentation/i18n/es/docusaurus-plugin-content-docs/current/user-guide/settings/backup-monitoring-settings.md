...
| **Frecuencia de notificaciones**      | Con qué frecuencia enviar notificaciones de vencimiento: <br/> **Una vez`: Send **just one** notification when the backup becomes overdue. <br/> `Todos los días`: Send **daily** notifications while overdue (default). <br/> `Cada semana`: Send **weekly** notifications while overdue. <br/> `Cada mes**: Envía notificaciones **mensuales** mientras esté vencido. <br/> Se registra un canal fallido, y la ventana de frecuencia sigue aplicándose cuando se entregó otro canal. Las alertas se repiten en cada comprobación de supervisión solo cuando fallan todos los canales habilitados. |
