# Refuerzo de seguridad {/* #security-hardening */}

El endurecimiento de producción para **duplistatus** es por capas y opcional. Cada característica descrita aquí está desactivada de forma predeterminada, por lo que una instalación nueva seguirá funcionando hasta que elijas activarla. Existen tres capas independientes:
...
1. Mantén el puerto `9666` fuera de internet público: enlaza la aplicación a localhost o a una red privada.
2. Crea [claves de API](#api-keys) y activa **Exigir claves de API para APIs externas**. Esto funciona sin un proxy inverso y es la ganancia más rápida.
3. Sirve **duplistatus** a través de un [proxy inverso con HTTPS](#https-with-a-reverse-proxy).
4. Añade la dirección TCP del par del proxy a **Proxies de confianza** (o `IP_TRUSTED_PROXIES`) si pretendes usar listas de permitidos.
5. Opcionalmente activa las [listas de IPs permitidas](#ip-allowlist) de administrador y externas, usando **IP detectada** y las sugerencias de IP recientes para evitar bloquearte a ti mismo.
...
La directiva `reverse_proxy` de Caddy establece los encabezados de IP del cliente por usted. Aún debe enumerar la dirección TCP del par del proxy bajo **Proxies de confianza** cuando utilice listas de IPs permitidas (consulte [abajo](#trusted-proxies-for-ip-allowlists)).
...
Cuando las [listas de IPs permitidas](../user-guide/settings/ip-allowlist-settings.md) están habilitadas, enumere el proxy bajo **Proxies de confianza** (o configure `IP_TRUSTED_PROXIES`). La aplicación respeta `X-Forwarded-For` y `X-Real-IP` solo cuando el par TCP es un proxy de confianza; de lo contrario los ignora.
