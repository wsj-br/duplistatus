...
La aplicación se puede implementar usando Docker, [Portainer Stacks](https://docs.portainer.io/user/docker/stacks) o Podman. Después de la instalación, es posible que desee configurar la ZONA HORARIA, como se describe en [Configurar zona horaria](./configure-tz.md) y debe configurar los servidores Duplicati para enviar registros de copia de seguridad a **duplistatus**, como se indica en la sección [Configuración de Duplicati](./duplicati-server-configuration.md).

## Requisitos Previos {/* #prerequisites */}

Asegúrese de tener lo siguiente instalado:

- Docker Engine - [Guía de instalación para Debian](https://docs.docker.com/engine/install/debian/)
- Docker Compose - [Guía de instalación para Linux](https://docs.docker.com/compose/install/linux/)
- Portainer (opcional) - [Guía de instalación para Docker](https://docs.portainer.io/start/install-ce/server/docker/linux)
...
Los administradores también pueden requerir opcionalmente [claves de API](../user-guide/settings/api-keys-settings.md) para Duplicati y Homepage, y restringir el acceso con [listas de permitidos IP](../user-guide/settings/ip-allowlist-settings.md). Ambos están desactivados por defecto.

::::info[IMPORTANTE]
El sistema aplica una longitud mínima y complejidad de contraseña. Estos requisitos se pueden ajustar utilizando las [variables de entorno](environment-variables.md) `PWD_ENFORCE` y `PWD_MIN_LEN`. Usar una contraseña sin suficiente complejidad o con una longitud corta puede comprometer la seguridad. Por favor, utilice estas configuraciones con cuidado.
::::
...
Este es el método recomendado para implementaciones locales o cuando desea personalizar la configuración. Utiliza un archivo `docker compose` para definir y ejecutar el contenedor con todos sus ajustes.
...
### Opción 3: Usando Portainer Stacks (Repositorio GitHub) {/* #option-3-using-portainer-stacks-github-repository */}
...
5. En el campo "Compose path", introduzca: `production.yml`
6. (opcional) Establezca las variables de entorno `TZ`, `LANG`, `PWD_ENFORCE` y `PWD_MIN_LEN` en la sección "Variables de entorno". Compruebe la sección [Zona horaria](./configure-tz.md) para obtener más detalles sobre cómo ajustar la zona horaria y el formato de número/fecha/hora. 
...
- El volumen `duplistatus_data` se utiliza para almacenamiento persistente. La imagen del contenedor usa `Europe/London` como zona horaria predeterminada y `en_GB` como configuración regional predeterminada (idioma).
...
Si necesita una configuración DNS personalizada (por ejemplo, para Tailscale MagicDNS, redes corporativas o configuraciones DNS personalizadas), puede configurar manualmente los servidores DNS y dominios de búsqueda.

**Búsqueda de su configuración DNS:**
...
Busque líneas que comiencen con `nameserver` (para servidores DNS) y `search` (para dominios de búsqueda). Si no está seguro de su configuración DNS o dominios de búsqueda de red, consulte con su administrador de red para obtener esta información.
...
**Nota**: Omita las direcciones IPv6 (que contienen `:`) y las direcciones locales (como `127.0.0.53`) al configurar servidores DNS.

Compruebe la sección [Zona horaria](./configure-tz.md) para obtener más detalles sobre cómo ajustar la zona horaria y el formato de número/fecha/hora.

### Opción 6: Usando Pods Podman {/* #option-6-using-podman-pods */}

Los pods Podman le permiten ejecutar múltiples contenedores en un espacio de nombres de red compartido. Esto es útil para pruebas o cuando necesita ejecutar duplistatus junto con otros contenedores.
...
#### Configuración de DNS para Pods Podman {/* #configuring-dns-for-podman-pods */}
...
1. Configure sus [servidores Duplicati](duplicati-server-configuration.md) para enviar mensajes de registro de copia de seguridad a duplistatus (obligatorio). En Duplicati 2.0.9.106 y versiones posteriores, utilice `--send-http-json-urls` como se describe en esa guía.
2. Inicie sesión en duplistatus: consulte las instrucciones en la [Guía de usuario](../user-guide/overview.md#accessing-the-dashboard).
3. Recopile registros iniciales de copia de seguridad: utilice la función [Recopilar registros de copias de seguridad](../user-guide/collect-backup-logs.md) para poblar la base de datos con datos históricos de copia de seguridad de todos sus servidores Duplicati. Esto también actualiza automáticamente los intervalos de monitoreo de copias de seguridad según la configuración de cada servidor.
4. Configure ajustes del servidor: configure alias y notas del servidor en [Configuración → Servidor](../user-guide/settings/server-settings.md) para hacer que su panel sea más informativo.
5. Configure la configuración de NTFY: configure notificaciones a través de NTFY en [Configuración → NTFY](../user-guide/settings/ntfy-settings.md).
