...
Los administradores también pueden, opcionalmente, requerir [claves de API](../user-guide/settings/api-keys-settings.md) para Duplicati y Homepage, y restringir el acceso con [listas de permitidos de IP](../user-guide/settings/ip-allowlist-settings.md). Ambos están desactivados por defecto.

::::info[IMPORTANTE]
El sistema impone una longitud mínima de contraseña y complejidad. Estos requisitos se pueden ajustar utilizando las variables de entorno `PWD_ENFORCE` y `PWD_MIN_LEN` [variables de entorno](environment-variables.md). Utilizar una contraseña sin la complejidad suficiente o con una longitud corta puede comprometer la seguridad. Por favor, utilice estos ajustes con cuidado.
::::
...
Este es el método recomendado para despliegues locales o cuando desea personalizar la configuración. Utiliza un archivo `docker compose` para definir y ejecutar el contenedor con todos sus ajustes.
...
2. Nombra tu stack (por ejemplo, "duplistatus").
...
5. En el campo "Componer ruta", ingrese: `production.yml`
...
6. Haga clic en "Desplegar el stack".
...
- El volumen `duplistatus_data` se usa para almacenamiento persistente. La imagen del contenedor usa `Europe/London` como la zona horaria predeterminada y `en_GB` como la configuración regional predeterminada (idioma).
...
#### Configurando DNS para contenedores de Podman {/* #configuring-dns-for-podman-containers */}
...
1. **Para sistemas con systemd-resolved** (la mayoría de las distribuciones Linux modernas):
...
Busque las líneas que comiencen con `nameserver` (para servidores DNS) y `search` (para dominios de búsqueda). Si no está seguro de su configuración de DNS o dominios de búsqueda de la red, consulte con su administrador de red para obtener esta información.
...
**Nota**: Omita las direcciones IPv6 (que contienen `:`) y las direcciones de localhost (como `127.0.0.53`) al configurar servidores DNS.
...
Al utilizar pods, la configuración de DNS debe establecerse a nivel de pod, no a nivel de contenedor.
...
1. Configure sus [servidores Duplicati](duplicati-server-configuration.md) para enviar mensajes de registro de copia de seguridad a duplistatus (obligatorio). En Duplicati 2.0.9.106 y posteriores, utilice `--send-http-json-urls` como se describe en esa guía.
...
3. Recopile los registros de copia de seguridad iniciales – utilice la función [Recopilar registros de copias de seguridad](../user-guide/collect-backup-logs.md) para poblar la base de datos con datos históricos de copia de seguridad de todos sus servidores Duplicati. Esto también actualiza automáticamente los intervalos de monitoreo de copia de seguridad según la configuración de cada servidor.
4. Configure los ajustes del servidor – configure alias de servidores y notas en [Configuración → Servidor](../user-guide/settings/server-settings.md) para hacer que su panel sea más informativo.
...
Consulte las siguientes secciones para configurar ajustes opcionales como la zona horaria, el formato de número y el [fortalecimiento de seguridad](security-hardening.md).
