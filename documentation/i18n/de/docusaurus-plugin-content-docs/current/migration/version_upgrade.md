---
translation_last_updated: '2026-01-31T00:51:25.979Z'
...
Dieser Leitfaden erklärt, wie Sie zwischen Versionen von duplistatus aktualisieren. Migrationen erfolgen automatisch – das Datenbankschema aktualisiert sich selbst, wenn Sie eine neue Version starten.

Manuelle Schritte sind nur erforderlich, wenn Sie benutzerdefinierte Benachrichtigungsvorlagen angepasst haben (Version 0.8.x hat Templatevariablen geändert) oder externe API-Integrationen aktualisiert werden müssen (Version 0.7.x hat API-Feldnamen geändert, Version 0.9.x erfordert Authentifizierung).
...
1. Navigieren Sie zu `Einstellungen → Datenbankverwaltung` in der Weboberfläche
...
Weitere Informationen finden Sie in der Dokumentation zur [Datenbankwartung](../user-guide/settings/database-maintenance.md#database-backup).
...
3. Klicken Sie auf die Registerkarte „Files".
...
Wenn Sie Ihre Datenbank aus einer vorherigen Sicherung wiederherstellen müssen, führen Sie die folgenden Schritte basierend auf Ihrem Betriebssystem durch.
...
##### Für Linux-Benutzer {#for-linux-users}
Die einfachste Möglichkeit zur Wiederherstellung besteht darin, die Sicherungsdatei in den internen Speicherplatzpfad des Containers „zurückzuschieben".
...
Benennen Sie die importierte Datei in genau backups.db um, wenn sie einen Zeitstempel im Namen enthält.
...
### Upgrade auf Version 0.9.x oder später (Schema v4.0) {#upgrading-to-version-09x-or-later-schema-v40}

:::warning
**Authentifizierung ist jetzt erforderlich.** Alle Benutzer müssen sich nach dem Upgrade anmelden.
:::

#### Was sich automatisch ändert {#what-changes-automatically}
...
- Alle bestehenden Sitzungen werden ungültig gemacht
...
2. **Passwort ändern** wenn aufgefordert (erforderlich beim ersten Anmelden)
...
Nach dem Upgrade bestätigen Sie:
...
1. Bestätigen Sie, dass die Sicherung erstellt wurde (Prüfen Sie das Datenverzeichnis)
...
2. Versuchen Sie die Standard-Anmeldedaten: `admin` / `Duplistatus09`
3. Verwenden Sie das Admin-Wiederherstellungstool, falls Sie gesperrt sind
...
3. Hinzufügen Sie Authentifizierung zu API-Anfragen (0.9.x+)
4. Testen Sie API-Endpunkte nach der Migration

### Probleme mit Master-Schlüsseln (0.8.x+) {#master-key-issues-08x}
...
4. **Vorherige Image-Version verwenden**: Rufen Sie die vorherige Container-Image ab und führen Sie diese aus
5. **Container starten**: Starten Sie mit der vorherigen Version

:::warning
Das Zurückrollen kann zu Datenverlust führen, wenn das neuere Schema mit der älteren Version nicht kompatibel ist. Stellen Sie immer sicher, dass Sie eine aktuelle Sicherung haben, bevor Sie versuchen, ein Rollback durchzuführen.
:::
...
* **Die Lösung:** Prüfen Sie das Verzeichnis `/app/data/`. Wenn Ihre Datei `duplistatus-backup-2024.db` heißt oder eine `.sqlite`-Erweiterung hat, wird die App sie ignorieren. Verwenden Sie den Befehl `mv` oder die Docker Desktop-GUI, um sie exakt in `backups.db` umzubenennen.

#### 3. Container wird nicht neu gestartet {#3-container-not-restarted}
...
Wenn Sie eine Sicherung von einer viel neueren Version von duplistatus in eine ältere Version der Anwendung wiederherstellen, ist das Datenbankschema möglicherweise nicht kompatibel.

* **Die Lösung:** Stellen Sie immer sicher, dass Sie die gleiche (oder eine neuere) Version des duplistatus-Images ausführen wie die Version, die die Sicherung erstellt hat. Prüfen Sie Ihre Version mit:
...
| Anwendungsversion | Schemaversion | Wichtigste Änderungen |
|----------------------------|----------------|----------------------------------------------------|
| 0.6.x und früher | v1.0 | Initiales Schema |
| 0.7.x | v2.0, v3.0 | Konfigurationen hinzugefügt, Server umbenannt |
