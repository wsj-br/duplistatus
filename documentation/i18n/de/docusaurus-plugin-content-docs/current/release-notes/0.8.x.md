---
translation_last_updated: '2026-02-06T22:33:32.594Z'
...
### Überwachung überfälliger Sicherungen {#enhanced-overdue-monitoring}
...
- **Neue Registerkarte `Overdue Monitoring` Konfiguration** mit Duplicati-Server-kompatiblen Intervalleinstellungen
    ...
    - Automatische Aktualisierung der überfälligen Intervalle aus der Duplicati-Konfiguration während der Sicherungsprotokollerfassung
...
- **QR-Code-Generierung** für automatische Gerätekonfiguration zum Empfangen von Benachrichtigungen von **duplistatus**
   ...
   - Schaltfläche „Gerät konfigurieren" in `Settings → NTFY Settings`
...
- **Server-Auswahlmenü** in der `Collect Backup Logs`-Schnittstelle für direkte serverspezifische Erfassung
...
- **Schaltflächen zur Sicherungserfassung für einzelne Server** hinzugefügt zu `Dashboard (Tabelle)`, `Einstellungen → Überfällige Überwachung` und `Einstellungen → Server`
- **Massenerfassungsfunktionalität** für alle Server mit gültiger Konfiguration (URL und Passwort)
   - `Alle erfassen`-Schaltflächen in `Einstellungen → Überfällige Überwachung` und `Einstellungen → Server`
   - Kontextmenü mit Rechtsklick auf die `Collect Backup Logs`-Schaltfläche in der Anwendungssymbolleiste

### Verbessertes Benachrichtigungssystem {#enhanced-notification-system}

- **SMTP-E-Mail-Benachrichtigungen-Unterstützung:**
    - SMTP-Server-Konfiguration in `Einstellungen → E-Mail-Einstellungen`
    - Pro-Sicherungs-Job-Konfiguration für NTFY und/oder E-Mail-Benachrichtigungen
    - HTML-formatierte Vorlagen unter Verwendung vorhandener `Einstellungen → Benachrichtigungsvorlagen`
- **Pro-Sicherungs-Job-Benachrichtigungskonfiguration:**
    - Individuelle Benachrichtigungseinstellungen in `Einstellungen → Sicherungsbenachrichtigungen`
    - Visuelle Indikatoren (ausgegraut Symbole), wenn NTFY oder E-Mail nicht ordnungsgemäß konfiguriert ist
