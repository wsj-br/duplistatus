---
translation_last_updated: '2026-01-31T00:51:26.138Z'
...
- **Gesamte Backup-Läufe**: Die Gesamtzahl der Sicherungsprotokolle von Läufen, die für alle Server empfangen oder gesammelt wurden.                                                                   |
...
- **Genutzter Speicherplatz**: Der gesamte Speicherplatz, der von Sicherungen am Sicherungsziel (z. B. Cloud-Speicher, FTP-Server, lokales Laufwerk) verwendet wird, basierend auf den neuesten Sicherungsprotokollen. |
- **Hochgeladene Größe**: Die Gesamtmenge der vom Duplicati-Server zum Ziel hochgeladenen Daten (z. B. lokaler Speicher, FTP, Cloud-Anbieter).                         |
...
- **Layout-Umschalter**: Wechselt zwischen dem Kartenlayout (Standard) und dem Tabellenlayout.                                                                                  |

## Kartenlayout {#cards-layout}

Das Kartenlayout zeigt den Status des neuesten Sicherungsprotokolleintrags an, der für jede Sicherung empfangen wurde.
...
- **Zusammenfassende Informationen**: Die konsolidierte Anzahl von Dateien, Größe und genutzter Speicherplatz für alle Sicherungen dieses Servers. Zeigt auch die verstrichene Zeit der zuletzt empfangenen Sicherung an (Mauszeiger darüber bewegen, um den Zeitstempel anzuzeigen)
...
- **Dauer, Dateianzahl, Dateigröße, Speichergröße, Hochgeladene Größe**: Werte wie vom duplistatus-Server gemeldet.
...
- **Erwartete Sicherung**: Der Zeitpunkt, zu dem die Sicherung erwartet wurde, einschließlich der konfigurierten Kulanzfrist (zusätzliche Zeit, die vor dem Markieren als überfällig gewährt wird).

Sie können auch auf die Schaltflächen am unteren Rand klicken, um `Einstellungen → Sicherungsbenachrichtigungen` zu öffnen, um überfällige Einstellungen zu konfigurieren, oder öffnen Sie die Weboberfläche des Duplicati-Servers.
...
- **Verfügbare Versionen**: Die Anzahl der auf dem Sicherungsziel gespeicherten Sicherungsversionen. Wenn das Symbol ausgegraut ist, wurden detaillierte Informationen nicht im Protokoll empfangen. Weitere Informationen finden Sie in der [Duplicati-Konfiguration](../installation/duplicati-server-configuration.md).
...
### Benachrichtigungen-Symbole {#notifications-icons}
...
- **Erwartete Sicherung**: Der Zeitpunkt, zu dem die Sicherung erwartet wurde, einschließlich der konfigurierten Kulanzfrist (zusätzliche Zeit, die vor dem Markieren als überfällig gewährt wird).
- **Letzte Benachrichtigung**: Wann die letzte Benachrichtigung über Überfälligkeit gesendet wurde.
...
Weitere Informationen finden Sie in der [Duplicati-Konfiguration](../installation/duplicati-server-configuration.md).
:::
