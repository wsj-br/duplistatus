---
translation_last_updated: '2026-01-31T00:51:26.263Z'
...
2.  Im Abschnitt **Datenbanksicherung** wählen Sie ein Sicherungsformat aus:
...
- **.db format**: Empfohlen für regelmäßige Sicherungen. Erstellt eine exakte Kopie der Datenbankdatei mit Hilfe der SQLite-Sicherungs-API und gewährleistet Konsistenz auch während die Datenbank in Verwendung ist.
...
1.  Navigieren Sie zu `Settings → Database Maintenance`.
2.  Im Abschnitt **Datenbankwiederherstellung** klicken Sie auf die Dateieingabe und wählen Sie eine Sicherungsdatei aus:
...
- Eine Sicherungssicherung von der aktuellen Datenbank wird automatisch vor der Wiederherstellung erstellt
...
- Alle Caches werden gelöscht, um frische Daten sicherzustellen

**Wiederherstellungsformate:**

- **.db Dateien**: Die Datenbankdatei wird direkt ersetzt. Schnellste Wiederherstellungsmethode.
- **.sql Dateien**: SQL-Anweisungen werden ausgeführt, um die Datenbank neu zu erstellen. Ermöglicht bei Bedarf eine selektive Wiederherstellung.
...
Eine Sicherungskopie wird automatisch erstellt, es wird jedoch empfohlen, vor der Wiederherstellung eine eigene Sicherung zu erstellen.
...
- Wenn die Wiederherstellung fehlschlägt, wird die ursprüngliche Datenbank automatisch aus der Sicherungskopie wiederhergestellt
...
:::note
Dies gilt für alle nachstehenden Wartungsfunktionen: Alle Statistiken im Dashboard, auf Detailseiten und in Diagrammen werden anhand von Daten aus der **duplistatus**-Datenbank berechnet. Das Löschen alter Informationen wirkt sich auf diese Berechnungen aus.
...
## Datenlöschzeitraum {#data-cleanup-period}
...
2.  Wählen Sie einen Aufbewahrungszeitraum:
    - **6 Monate**: Datensätze aus den letzten 6 Monaten beibehalten.
    - **1 Jahr**: Datensätze aus dem letzten Jahr beibehalten.
    - **2 Jahre**: Datensätze aus den letzten 2 Jahren beibehalten (Standard).
...
Entfernen Sie Daten eines bestimmten Sicherungs-Job-Typs.
...
- Löscht alle Daten, die mit diesem Sicherungsauftrag / Server verknüpft sind, dauerhaft.
...
1.  Navigieren Sie zu `Settings → Database Maintenance`.
2.  Auswählen Sie einen Server aus der Dropdown-Liste.
...
Dies kann auftreten, wenn sich die `machine-id` von Duplicati nach einem Upgrade oder einer Neuinstallation ändert. Doppelte Server werden nur angezeigt, wenn sie vorhanden sind. Wenn keine Duplikate erkannt werden, zeigt der Abschnitt eine Meldung an, die angibt, dass alle Server eindeutige Namen haben.
...
5.  Klicken Sie auf <IconButton icon="lucide:git-merge" label="Merge Selected Servers" />.
6.  Bestätigen Sie die Aktion im Dialogfeld.

**Zusammenführungsprozess:**

- Alle alten Server-IDs werden auf dem Zielserver zusammengeführt (neuester nach Erstellungsdatum)
- Alle Sicherungsdatensätze und Konfigurationen werden auf den Zielserver übertragen
