---
translation_last_updated: '2026-02-14T04:57:43.214Z'
...
2. Die Tabelle ist nach Server gruppiert, mit unterschiedlichen Server-Header-Zeilen, die den Servernamen, den Alias und die Anzahl der Sicherungen anzeigen.
   - **Hinweis**: Bei Servern mit nur einer Sicherung wird stattdessen eine zusammengeführte Zeile angezeigt. Server-Standardwerte können nicht direkt aus zusammengeführten Zeilen konfiguriert werden. Wenn Sie Server-Standardwerte für einen Server mit einer Sicherung konfigurieren müssen, können Sie dies tun, indem Sie vorübergehend eine weitere Sicherung zu diesem Server hinzufügen, oder die zusätzlichen Ziele der Sicherung erben automatisch von vorhandenen Server-Standardwerten.
...
- Wann eine Sicherung von Server-Standardwerten erbt, werden alle Änderungen an den Server-Standardwerten automatisch auf diese Sicherung angewendet (sofern sie nicht überschrieben wurde).
...
3. Sobald Sicherungen ausgewählt sind, wird eine Massenaktionsleiste angezeigt, die die Anzahl der ausgewählten Sicherungen zeigt.
