...
3. **Update glossary** (if intlayer translations changed): `pnpm run translate:glossary-ui`
...
- **`.translate-ignore`**: Gitignore-style patterns for files to skip during AI translation. Includes both documentation files (paths relative to `docs/`) and SVG files (basenames in `static/img/`). Examples: `api-reference/*`, `LICENSE.md`, `duplistatus_logo.svg`
...
`glossary-user.csv` lets you override or add terms without modifying the generated UI glossary. Format: `en`, `locale`, `translation` (one row per term per locale). Entries take precedence over `glossary-ui.csv`.
...
Options: `--locale`, `--path`, `--dry-run`, `--no-cache`, `--force`, `--no-export-png`. Uses `.translate-ignore` for exclusions. Optionally exports PNG via Inkscape CLI.

### Workflow with AI Translation

1. **Update English documentation** in `docs/`
2. **Update glossary** (if needed): `pnpm run translate:glossary-ui`
