...
### Critical: Overdue Backup Detection - Complete Reimplementation {#critical-overdue-backup-detection-complete-reimplementation}
...
- **Fixed overdue backup detection** - Completely reimplemented overdue backup detection system with a new algorithm that properly tracks expected backup dates:
  ...
  - Fixed "Next Run" column in backup monitoring settings showing incorrect dates
...
### From Version 1.0.x {#from-version-10x}
...
3. **Overdue Backup Detection** - The overdue backup detection system has been completely reimplemented with a new algorithm that preserves schedule time-of-day. Existing backup monitoring configurations will continue to work, but you may notice more accurate "Next Run" dates in the backup monitoring settings. The new algorithm ensures that manual backups don't shift scheduled backup times
...
### Version 1.1.0 Changes {#version-110-changes}
