...
| **Daily Summary**  | Markdown email template for the optional daily snapshot. |
...
| <IconButton icon="lucide:send" label="Send Test Notification"/>     | Checks the template after updating it. The variables will be replaced with their names for the test. For email notifications, the template title becomes the email subject line. Not available on the Daily Summary tab. |
