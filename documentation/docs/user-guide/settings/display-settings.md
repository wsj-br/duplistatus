...
| **Format Locale**         | Select a formatting locale independent of your UI language (416 locales supported). This affects how dates, times, and numbers are displayed. Relative-time phrases (for example "in 2 hours") follow Format Locale unless **Keep relative time in the UI language** is enabled. A live preview includes date, time, number, and a relative-time sample. Example: UI language = German, Format locale = English (UK) → German UI with UK date formats. | Based on UI language |
| **Keep relative time in the UI language** | When enabled, phrases such as "in 2 hours" follow **User → Language** instead of Format Locale. Dates, times, and numbers still use Format Locale. Off by default. | Off |
