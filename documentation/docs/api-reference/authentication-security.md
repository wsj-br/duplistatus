...
- `/api/health` - Health check endpoint (never keyed; cheap SQLite probe; per-IP rate limit)
- `/api/ping` - Connectivity probe (never keyed; per-IP rate limit)

When **Require API keys** is off, the first four routes accept requests with or without a key: a valid matching-scope key is recorded; a bad key is ignored. When the switch is on, they return `401` without a valid key and `403` when the key scope does not match. `/api/health` and `/api/ping` never use keys. See [API Keys](../user-guide/settings/api-keys-settings.md) and [IP Allowlist](../user-guide/settings/ip-allowlist-settings.md).
